...
class ClassGoogleTakeout:
    ...
    Replaces global LOGGER references with LOGGER, preserving the original log text.
    """
    ...
    def google_takeout_processor(self, output_takeout_folder, log_level=logging.INFO):
        ...
        with set_log_level(LOGGER, log_level):  # Temporarily adjust log level
            ...
            if not os.path.isdir(self.ARGS['google-input-takeout-folder']):
                LOGGER.error(f"ERROR   : Cannot Find INPUT_FOLDER: '{self.ARGS['google-input-takeout-folder']}'. Exiting...")
                sys.exit(-1)
            ...
            if self.ARGS['google-skip-gpth-tool'] or self.ARGS['google-ignore-check-structure']:
                ...
                if self.ARGS['google-skip-gpth-tool']:
                    LOGGER.warning(f"WARNING : Metadata fixing with GPTH tool skipped ('-gsgt, --google-skip-gpth-tool' flag). step {step}b is needed to copy files manually to output folder.")
                elif self.ARGS['google-ignore-check-structure']:
                    LOGGER.warning(f"WARNING : Flag to Ignore Google Takeout Structure detected. step {step}b is needed to copy/move files manually to output folder.")
                if self.ARGS['google-move-takeout-folder']:
            ...
            if (self.ARGS['google-albums-folders-structure'].lower() == 'flatten'
                ...
                LOGGER.info("")
                LOGGER.warning("WARNING : No argument '-as, --google-albums-folders-structure' and '-ns, --google-no-albums-folder-structure' detected. All photos and videos will be flattened in their folders.")
            else:
                step_end_time = datetime.now()
                formatted_duration = str(timedelta(seconds=(step_end_time - step_start_time).seconds))
                LOGGER.info(f"INFO    : step {step} completed in {formatted_duration}.")
