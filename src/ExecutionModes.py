...
def mode_google_takeout(user_confirmation=True, log_level=None):
    ...
    with set_log_level(LOGGER, log_level):  # Change Log Level to log_level for this function
        ...
        diff_output_input_total_files               = abs(result['output_counters']['total_files']           - result['input_counters']['total_files'])             if result['input_counters']['total_files']           != 0 and result['output_counters']['total_files']           != 0 else 0
        diff_output_input_total_unsupported_files   = abs(result['output_counters']['unsupported_files']     - result['input_counters']['unsupported_files'])       if result['input_counters']['unsupported_files']     != 0 and result['output_counters']['unsupported_files']     != 0 else 0
        diff_output_input_total_supported_files     = abs(result['output_counters']['supported_files']       - result['input_counters']['supported_files'])         if result['input_counters']['supported_files']       != 0 and result['output_counters']['supported_files']       != 0 else 0
        diff_output_input_total_media               = abs(result['output_counters']['media_files']           - result['input_counters']['media_files'])             if result['input_counters']['media_files']           != 0 and result['output_counters']['media_files']           != 0 else 0
        diff_output_input_total_images              = abs(result['output_counters']['photo_files']           - result['input_counters']['photo_files'])             if result['input_counters']['photo_files']           != 0 and result['output_counters']['photo_files']           != 0 else 0
        diff_output_input_total_photos_with_date    = abs(result['output_counters']['photos']['with_date']   - result['input_counters']['photos']['with_date'])     if result['input_counters']['photos']['with_date']   != 0 and result['output_counters']['photos']['with_date']   != 0 else 0
        diff_output_input_total_photos_without_date = abs(result['output_counters']['photos']['without_date']- result['input_counters']['photos']['without_date'])  if result['input_counters']['photos']['without_date']!= 0 and result['output_counters']['photos']['without_date']!= 0 else 0
        diff_output_input_total_videos              = abs(result['output_counters']['video_files']           - result['input_counters']['video_files'])             if result['input_counters']['video_files']           != 0 and result['output_counters']['video_files']           != 0 else 0
        diff_output_input_total_videos_with_date    = abs(result['output_counters']['videos']['with_date']   - result['input_counters']['videos']['with_date'])     if result['input_counters']['videos']['with_date']   != 0 and result['output_counters']['videos']['with_date']   != 0 else 0
        diff_output_input_total_videos_without_date = abs(result['output_counters']['videos']['without_date']- result['input_counters']['videos']['without_date'])  if result['input_counters']['videos']['without_date']!= 0 and result['output_counters']['videos']['without_date']!= 0 else 0
        diff_output_input_total_non_media           = abs(result['output_counters']['non_media_files']       - result['input_counters']['non_media_files'])         if result['input_counters']['non_media_files']       != 0 and result['output_counters']['non_media_files']       != 0 else 0
        diff_output_input_total_metadata            = abs(result['output_counters']['metadata_files']        - result['input_counters']['metadata_files'])          if result['input_counters']['metadata_files']        != 0 and result['output_counters']['metadata_files']        != 0 else 0
        diff_output_input_total_sidecars            = abs(result['output_counters']['sidecar_files']         - result['input_counters']['sidecar_files'])           if result['input_counters']['sidecar_files']         != 0 and result['output_counters']['sidecar_files']         != 0 else 0
        ...
        if result['output_counters']['total_files'] == 0:
            ...
            LOGGER.info(f"      - With Date                           : {result['input_counters']['photos']['with_date']:<7} ({result['input_counters']['photos']['pct_with_date']:>5.1f}% of total photos) ")
            LOGGER.info(f"      - Without Date                        : {result['input_counters']['photos']['without_date']:<7} ({result['input_counters']['photos']['pct_without_date']:>5.1f}% of total photos) ")
            LOGGER.info(f"    - Total Videos in Takeout folder        : {result['input_counters']['video_files']:<7}")
            LOGGER.info(f"      - With Date                           : {result['input_counters']['videos']['with_date']:<7} ({result['input_counters']['videos']['pct_with_date']:>5.1f}% of total videos) ")
            LOGGER.info(f"      - Without Date                        : {result['input_counters']['videos']['without_date']:<7} ({result['input_counters']['videos']['pct_without_date']:>5.1f}% of total videos) ")
            ...
            LOGGER.info(f"------------------------------------------------------------------------------------------------------------------")
            LOGGER.info(f"Total Files in Output folder                : {result['output_counters']['total_files']:<7} (diff: {diff_output_input_total_files:>5}) | ({perc_of_input_total_files:>5.1f}% of input) ")
            LOGGER.info(f"Total Non-Supported files in Output folder  : {result['output_counters']['unsupported_files']:<7} (diff: {diff_output_input_total_unsupported_files:>5}) | ({perc_of_input_total_unsupported_files:>5.1f}% of input) ")
            LOGGER.info(f"Total Supported files in Output folder      : {result['output_counters']['supported_files']:<7} (diff: {diff_output_input_total_supported_files:>5}) | ({perc_of_input_total_supported_files:>5.1f}% of input) ")
            LOGGER.info(f"  - Total Media files in Output folder      : {result['output_counters']['media_files']:<7} (diff: {diff_output_input_total_media:>5}) | ({perc_of_input_total_media:>5.1f}% of input) ")
            LOGGER.info(f"    - Total Images in Output folder         : {result['output_counters']['photo_files']:<7} (diff: {diff_output_input_total_images:>5}) | ({perc_of_input_total_images:>5.1f}% of input) ")
            LOGGER.info(f"      - With Date                           : {result['output_counters']['photos']['with_date']:<7} (diff: {diff_output_input_total_photos_with_date:>5}) | ({perc_of_input_total_photos_with_date:>5.1f}% of input) | ({result['output_counters']['photos']['pct_with_date']:>5.1f}% of total photos) ")
            LOGGER.info(f"      - Without Date                        : {result['output_counters']['photos']['without_date']:<7} (diff: {diff_output_input_total_photos_without_date:>5}) | ({perc_of_input_total_photos_without_date:>5.1f}% of input) | ({result['output_counters']['photos']['pct_without_date']:>5.1f}% of total photos) ")
            LOGGER.info(f"    - Total Videos in Output folder         : {result['output_counters']['video_files']:<7} (diff: {diff_output_input_total_videos:>5}) | ({perc_of_input_total_videos:>5.1f}% of input) ")
            LOGGER.info(f"      - With Date                           : {result['output_counters']['videos']['with_date']:<7} (diff: {diff_output_input_total_videos_with_date:>5}) | ({perc_of_input_total_videos_with_date:>5.1f}% of input) | ({result['output_counters']['videos']['pct_with_date']:>5.1f}% of total videos) ")
            LOGGER.info(f"      - Without Date                        : {result['output_counters']['videos']['without_date']:<7} (diff: {diff_output_input_total_videos_without_date:>5}) | ({perc_of_input_total_videos_without_date:>5.1f}% of input) | ({result['output_counters']['videos']['pct_without_date']:>5.1f}% of total videos) ")
            LOGGER.info(f"  - Total Non-Media files in Output folder  : {result['output_counters']['non_media_files']:<7} (diff: {diff_output_input_total_non_media:>5}) | ({perc_of_input_total_non_media:>5.1f}% of input) ")
            LOGGER.info(f"    - Total Metadata in Output folder       : {result['output_counters']['metadata_files']:<7} (diff: {diff_output_input_total_metadata:>5}) | ({perc_of_input_total_metadata:>5.1f}% of input) ")
            LOGGER.info(f"    - Total Sidecars in Output folder       : {result['output_counters']['sidecar_files']:<7} (diff: {diff_output_input_total_sidecars:>5}) | ({perc_of_input_total_sidecars:>5.1f}% of input) ")
            LOGGER.info(f"------------------------------------------------------------------------------------------------------------------")
            LOGGER.info(f"")
            LOGGER.info(f"------------------------------------------------------------------------------------------------------------------")
