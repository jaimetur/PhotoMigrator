...
class ClassImmichPhotos:
    ...
    def read_config_file(self, config_file='Config.ini', log_level=logging.INFO):
        """
        Reads IMMICH configuration (IMMICH_URL, IMMICH_USERNAME, IMMICH_PASSWORD, etc.)
        from a config file. If a user API key is found, sets self.API_KEY_LOGIN=True.
        """
        ...
        with set_log_level(self.logger, log_level):  # Change Log Level to log_level for this function
            ...
            if not self.IMMICH_URL or self.IMMICH_URL.strip() == '':
                self.logger.warning(f"WARNING : IMMICH_URL not found. It will be requested on screen.")
                self.CONFIG['IMMICH_URL'] = input("[PROMPT] Enter IMMICH_URL (e.g., http://192.168.1.100:2283): ")
                self.IMMICH_URL = CONFIG['IMMICH_URL']
            if not self.IMMICH_USER_API_KEY or self.IMMICH_USER_API_KEY.strip() == '':
                if not self.IMMICH_USERNAME or self.IMMICH_USERNAME.strip() == '':
                    self.logger.warning(f"WARNING : IMMICH_USERNAME not found. It will be requested on screen.")
                ...
                if not self.IMMICH_PASSWORD or self.IMMICH_PASSWORD.strip() == '':
                    self.logger.warning(f"WARNING : IMMICH_PASSWORD not found. It will be requested on screen.")
                ...
                self.logger.info("")
                self.logger.info(f"INFO    : Immich Config Read:")
                self.logger.info(f"INFO    : -------------------")
                self.logger.info(f"INFO    : IMMICH_URL            : {self.IMMICH_URL}")
                if self.API_KEY_LOGIN:
                    masked_admin_api = '*' * len(self.IMMICH_ADMIN_API_KEY)
                    masked_user_api = '*' * len(self.IMMICH_USER_API_KEY)
                    self.logger.info(f"INFO    : IMMICH_ADMIN_API_KEY  : {masked_admin_api}")
                    self.logger.info(f"INFO    : IMMICH_USER_API_KEY   : {masked_user_api}")
                else:
                    self.logger.info(f"INFO    : IMMICH_USERNAME       : {self.IMMICH_USERNAME}")
                    masked_password = '*' * len(IMMICH_PASSWORD)
                    self.logger.info(f"INFO    : IMMICH_PASSWORD       : {masked_password}")
                self.logger.info(f"INFO    : IMMICH_FILTER_ARCHIVE : {self.IMMICH_FILTER_ARCHIVE}")
                self.logger.info(f"INFO    : IMMICH_FILTER_FROM    : {self.IMMICH_FILTER_FROM}")
                self.logger.info(f"INFO    : IMMICH_FILTER_TO      : {self.IMMICH_FILTER_TO}")
                self.logger.info(f"INFO    : IMMICH_FILTER_COUNTRY : {self.IMMICH_FILTER_COUNTRY}")
                self.logger.info(f"INFO    : IMMICH_FILTER_CITY    : {self.IMMICH_FILTER_CITY}")
                self.logger.info(f"INFO    : IMMICH_FILTER_PERSON  : {self.IMMICH_FILTER_PERSON}")
    ...
    def login(self, log_level=logging.INFO):
        ...
        with set_log_level(self.logger, log_level):
            ...
            self.logger.info("")
            self.logger.info(f"INFO    : Authenticating on Immich Photos and getting Session...")
            ...
            if self.API_KEY_LOGIN:
                ...
                try:
                    ...
                    self.logger.error(f"ERROR   : Exception occurred during Immich login: {str(e)}")
                    return False
                ...
                self.logger.info(f"INFO    : Authentication Successfully with IMMICH_USER_API_KEY found in Config file.")
            else:
                # Using user/password
                ...
                if not self.SESSION_TOKEN:
                    self.logger.error(f"ERROR   : 'accessToken' not found in the response: {data}")
                    return False
                ...
                self.logger.info(f"INFO    : Authentication Successfully with user/password found in Config file.")
    ...
    def logout(self, log_level=logging.INFO):
        ...
        (Immich does not provide an official /logout endpoint).
        """
        with set_log_level(self.logger, log_level):
            self.SESSION_TOKEN = None
            self.HEADERS_WITH_CREDENTIALS = {}
            self.logger.info("INFO    : Session closed locally (Bearer Token discarded).")
    ...
    def get_supported_media_types(self, type='media', log_level=logging.INFO):
        """
        Returns the supported media/sidecar extensions as reported by Immich
        (via /api/server/media-types).
        """
        with set_log_level(self.logger, log_level):
            ...
            try:
                ...
                if type.lower() == 'media':
                    ...
                    self.logger.debug(f"DEBUG   : Supported media types: '{supported_types}'.")
                elif type.lower() == 'image':
                    supported_types = image
                    self.logger.debug(f"DEBUG   : Supported image types: '{supported_types}'.")
                elif type.lower() == 'video':
                    supported_types = video
                    self.logger.debug(f"DEBUG   : Supported video types: '{supported_types}'.")
                elif type.lower() == 'sidecar':
                    supported_types = sidecar
                    self.logger.debug(f"DEBUG   : Supported sidecar types: '{supported_types}'.")
                else:
                    self.logger.error(f"ERROR   : Invalid type '{type}' to get supported media types. Types allowed are 'media', 'image', 'video' or 'sidecar'")
                    return None
                return supported_types
            except Exception as e:
                self.logger.error(f"ERROR   : Cannot get Supported media types: {e}")
                return None
    ...
    def get_user_id(self, log_level=logging.INFO):
        ...
        with set_log_level(self.logger, log_level):
            ...
            try:
                ...
                self.logger.info(f"INFO    : User ID: '{user_id}' found for user '{user_mail}'.")
                return user_id
            except Exception as e:
                self.logger.error(f"ERROR   : Cannot find User ID for user '{self.IMMICH_USERNAME}': {e}")
                return None
    ...
    def create_album(self, album_name, log_level=logging.INFO):
        ...
        Returns the ID or None if it fails.
        """
        with set_log_level(self.logger, log_level):
            self.login(log_level=log_level)
            ...
            try:
                ...
                self.logger.debug(f"DEBUG   : Album '{album_name}' created with ID: {album_id}")
                return album_id
            except Exception as e:
                self.logger.warning(f"WARNING : Cannot create album '{album_name}' due to API call error. Skipped! {e}")
                return None


    def remove_album(self, album_id, album_name, log_level=logging.INFO):
        """
        Removes an album from Immich by its ID. Returns True on success, False otherwise.
        """
        with set_log_level(self.logger, log_level):
            self.login(log_level=log_level)
            ...
            try:
                ...
                self.logger.error(f"ERROR   : Error while removing album '{album_name}' with ID:  {album_id}: {e}")
                return False


    def get_albums(self, log_level=logging.INFO):
        """
        Returns the list of albums for the current user in Immich.
        Each item is a dictionary with fields like:
            {
              "id": <str>,
              "albumName": <str>,
              "ownerId": <str>,
    ...
    def add_assets_to_album(self, album_id, asset_ids, album_name=None, log_level=logging.INFO):
        """
        Adds a list of asset_ids (photos/videos) to the specified album.
        ...
        with set_log_level(self.logger, log_level):
            ...
            try:
                ...
                if album_name:
                    self.logger.warning(f"WARNING : Error while adding assets to album '{album_name}' with ID={album_id}: {e}")
                else:
                    self.logger.warning(f"WARNING : Error while adding assets to album with ID={album_id}: {e}")
                return 0
    ...
    def remove_assets(self, assets_ids, log_level=logging.INFO):
        """
        Removes a list of assets given by their IDs.
        Returns the number of assets removed.
        """
        with set_log_level(self.logger, log_level):
            self.login(log_level=log_level)
            url = f"{self.IMMICH_URL}/api/assets"
            payload = json.dumps({"force": True, "ids": assets_ids})
            try:
                response = requests.delete(url, headers=self.HEADERS_WITH_CREDENTIALS, data=payload)
                response.raise_for_status()
                if response.ok:
                    return len(assets_ids)
                else:
                    self.logger.error(f"ERROR   : Failed to remove assets due to API error")
                    return 0
            except Exception as e:
                self.logger.error(f"ERROR   : Failed to remove assets: {str(e)}")
                return 0
    ...
    def remove_duplicates_assets(self, log_level=logging.INFO):
        ...
        with set_log_level(self.logger, log_level):
            ...
            if len(duplicates_ids) > 0:
                self.logger.info(f"INFO    : Removing Duplicates Assets...")
    ...
    def upload_asset(self, file_path, log_level=logging.INFO):
        ...
        with set_log_level(self.logger, log_level):
            ...
            try:
                ...
                if asset_id:
                    if is_duplicated:
                        self.logger.debug(f"DEBUG   : Duplicated Asset: '{os.path.basename(file_path)}'. Skipped!")
                    else:
                        self.logger.debug(f"DEBUG   : Uploaded '{os.path.basename(file_path)}' with asset_id={asset_id}")
                return asset_id, is_duplicated
            except Exception as e:
                self.logger.error(f"ERROR   : Failed to upload '{file_path}': {e}")
                return None, None


    def download_asset(self, asset_id, asset_filename, asset_time,
                       download_folder="Downloaded_Immich", log_level=logging.INFO):
        """
        Downloads an asset (photo/video) from Immich into a local folder.
        Returns True if successful, False otherwise.
        """
        with set_log_level(self.logger, log_level):
            self.login(log_level=log_level)
            ...
            try:
                ...
                self.logger.error(f"ERROR   : Failed to download asset {asset_id}: {e}")
                return False
    ...
    def upload_albums(self, input_folder, subfolders_exclusion='No-Albums',
        ...
        """
        Traverses the subfolders of 'input_folder', creating an album for each valid subfolder.
        Then uploads all valid files, associating them to the album. Maintains original LOGGER lines.
        """
        with set_log_level(self.logger, log_level):
            self.login(log_level=log_level)
            if not os.path.isdir(input_folder):
                self.logger.error(f"ERROR   : The folder '{input_folder}' does not exist.")
                self.logout(log_level=log_level)
                return 0, 0, 0, 0, 0
            ...
            total_dupplicated_assets_skipped = 0
                ...
                for subpath in valid_folders:
                    ...
                    for f in os.listdir(subpath):
                        ...
                        if is_dup:
                            ...
                            self.logger.debug(f"DEBUG   : Dupplicated Asset: {file_path}. Asset ID: {asset_id} skipped")
                        elif asset_id:
                    ...
                    if album_assets_ids:
                        ...
                        if not album_id:
                            ...
                            self.logger.debug(f"DEBUG   : Album '{album_name}' created with ID: {album_id}. Total Assets added to Album: {len(album_assets_ids)}.")
            ...
            self.logger.info(f"INFO    : Uploaded {total_albums_uploaded} album(s) from '{input_folder}'.")
            self.logger.info(f"INFO    : Uploaded {total_assets_uploaded} asset(s) from '{input_folder}' to Albums.")
            self.logger.info(f"INFO    : Skipped {total_albums_skipped} album(s) from '{input_folder}'.")
            self.logger.info(f"INFO    : Skipped {total_dupplicated_assets_skipped} duplicated asset(s) from '{input_folder}' to Albums.")
            self.logger.info(f"INFO    : Removed {total_duplicates_assets_removed} duplicates asset(s) from Immich Database.")
        ...
        with set_log_level(self.logger, log_level):
                ...
                if only_subfolders:
                    for sub in only_subfolders:
                        ...
                        if not os.path.isdir(sub_path):
                            self.logger.warning(f"WARNING : Subfolder '{sub}' does not exist in '{base_folder}'. Skipping.")
                            continue
            ...
            with tqdm(total=total_files, smoothing=0.1,
                ...
                for f_idx, file_path in enumerate(file_paths, start=1):
                    ...
                    ext = ext.lower()
                    if ext not in self.ALLOWED_IMMICH_EXTENSIONS:
                        self.logger.debug(f"DEBUG   : Unsopported Extension: '{ext}'. Skipped")
                        continue
                    ...
                    if is_dup:
                        ...
                        self.logger.debug(f"DEBUG   : Asset ID: {asset_id} uploaded to Immich Photos")
                        total_assets_uploaded += 1
            ...
            self.logger.info(f"INFO    : Uploaded {total_assets_uploaded} files (without album) from '{input_folder}'.")
            self.logger.info(f"INFO    : Skipped {total_dupplicated_assets_skipped} duplicated asset(s) from '{input_folder}'.")
            self.logger.info(f"INFO    : Removed {duplicates_assets_removed} duplicates asset(s) from Immich Database.")

            self.logout(log_level=log_level)
            return total_assets_uploaded, total_dupplicated_assets_skipped, duplicates_assets_removed


    def upload_ALL(self, input_folder, albums_folders=None,
                   remove_duplicates=False, log_level=logging.WARNING):
        """
        Uploads ALL photos/videos from 'input_folder' into Immich:
         1) Subfolders as albums
         2) The rest as no-album
        Returns several counters about how many albums/assets were uploaded.
        """
        with set_log_level(self.logger, log_level):
            self.login(log_level=log_level)
            ...
            (total_albums_uploaded, total_albums_skipped,
             ...
             duplicates_assets_removed_1) = self.upload_albums(
            ...
            (total_assets_uploaded_without_albums,
             ...
             duplicates_assets_removed_2) = self.upload_no_albums(
                 input_folder=input_folder,
            ...
            if remove_duplicates:
                self.logger.info("INFO    : Removing Duplicates Assets...")
                duplicates_assets_removed = self.remove_duplicates_assets(log_level=logging.WARNING)
    ...
    def download_albums(self, albums_name='ALL', output_folder="Downloads_Immich",
                        log_level=logging.WARNING):
        """
        Downloads photos/videos from albums by name pattern or ID. 'ALL' downloads all.
        Preserves original LOGGER lines.
        """
        with set_log_level(self.logger, log_level):
            self.login(log_level=log_level)
            ...
            all_albums = self.get_albums(log_level=log_level)
            if not all_albums:
                self.logger.warning("WARNING : No albums available or could not retrieve the list.")
                self.logout(log_level=log_level)
                return 0, 0
            ...
            if 'ALL' in [x.strip().upper() for x in albums_name]:
                ...
                self.logger.info(f"INFO    : ALL albums ({len(all_albums)}) will be downloaded...")
            else:
                found_albums = []
                ...
                if found_albums:
                    ...
                    self.logger.info(f"INFO    : {len(found_albums)} album(s) matched pattern(s) '{albums_name}'.")
                else:
                    self.logger.warning(f"WARNING : No albums found matching pattern(s) '{albums_name}'.")
                    self.logout(log_level=log_level)
                    return 0, 0
            ...
            for alb in albums_to_download:
                ...
                for asset in tqdm(assets_in_album, desc=f"INFO    : Downloading '{alb_name}'", unit=" assets"):
                    ...
                    if asset_id:
                        ...
                        ok = self.download_asset(asset_id, asset_filename, asset_time, alb_folder, log_level=log_level)
                        if ok:
                            total_assets_downloaded += 1

                total_albums_downloaded += 1
                self.logger.info(f"INFO    : Downloaded Album [{total_albums_downloaded}/{total_albums}] - '{alb_name}'. {len(assets_in_album)} asset(s) have been downloaded.")

            self.logger.info(f"INFO    : Download of Albums completed.")
            self.logger.info(f"INFO    : Total Albums downloaded: {total_albums_downloaded}")
            self.logger.info(f"INFO    : Total Assets downloaded: {total_assets_downloaded}")
    ...
    def download_no_albums(self, output_folder="Downloads_Immich", log_level=logging.WARNING):
        """
        Downloads all photos/videos that are NOT in any album, into
        output_folder/No-Albums/yyyy/mm.
        Preserves original LOGGER lines.
        """
        with set_log_level(self.logger, log_level):
            self.login(log_level=log_level)
            ...
            for asset in tqdm(all_assets_items, desc="INFO    : Downloading assets without associated albums", unit=" photos"):
                ...
                asset_time = asset.get('fileCreatedAt')
                ok = self.download_asset(asset_id, asset_filename, asset_time, target_folder, log_level=log_level)
                if ok:
                    total_assets_downloaded += 1

            self.logger.info(f"INFO    : Download of assets without associated albums completed.")
            self.logger.info(f"INFO    : Total Assets downloaded: {total_assets_downloaded}")
    ...
    def download_ALL(self, output_folder="Downloads_Immich", log_level=logging.WARNING):
        """
        Downloads ALL photos/videos from Immich:
          - First all albums to output_folder/Albums/<albumName>/
          - Then any assets not in an album to output_folder/No-Albums/yyyy/mm
        """
        with set_log_level(self.logger, log_level):
            self.login(log_level=log_level)
            ...
            self.logger.info(f"INFO    : Download of ALL assets completed.")
            self.logger.info(f"Total Albums downloaded                   : {total_albums_downloaded}")
            self.logger.info(f"Total Assets downloaded                   : {total_assets}")
            self.logger.info(f"Total Assets downloaded within albums     : {total_assets_in_albums}")
            self.logger.info(f"Total Assets downloaded without albums    : {total_assets_no_albums}")
    ...
    def remove_empty_albums(self, log_level=logging.WARNING):
        ...
        with set_log_level(self.logger, log_level):
            self.login(log_level=log_level)
            albums = self.get_albums(log_level=log_level)
            if not albums:
    ...
    def remove_duplicates_albums(self, log_level=logging.WARNING):
        ...
        keeping only the first album in each duplicate group.
        """
        with set_log_level(self.logger, log_level):
            self.login(log_level=log_level)
            albums = self.get_albums(log_level=log_level)
    ...
    def remove_all_assets(self, log_level=logging.WARNING):
        """
        Removes ALL assets in Immich (in batches of 250) and then removes empty albums.
        ...
        with set_log_level(self.logger, log_level):
            ...
            if total_assets_found == 0:
                self.logger.warning(f"WARNING : No Assets found in Immich Database.")
            self.logger.info(f"INFO    : Found {total_assets_found} asset(s) to remove.")
            ...
            self.logger.info(f"INFO    : Getting empty albums to remove...")
            total_removed_albums = self.remove_empty_albums(log_level=logging.WARNING)

            self.logout(log_level=log_level)
            self.logger.info(f"INFO    : Total Assets removed: {total_removed_assets}")
            self.logger.info(f"INFO    : Total Albums removed: {total_removed_albums}")
    ...
    def remove_all_albums(self, removeAlbumsAssets=False, log_level=logging.WARNING):
        ...
        with set_log_level(self.logger, log_level):
            ...
            self.logger.info(f"INFO    : Removed {total_removed_albums} albums.")
            if removeAlbumsAssets:
                self.logger.info(f"INFO    : Removed {total_removed_assets} assets associated to albums.")
