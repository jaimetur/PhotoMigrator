    ...
    def _format_action(self, action):
        ...
        if action.help:
            ...
            if help_text.find("Process the Takeout folder <TAKEOUT_FOLDER> to fix all metadata")!=-1:
                ...
                If only the argument -gTakeout, --google-takeout-to-process <TAKEOUT_FOLDER> is detected, then the Tool will use the default values for the rest of the arguments for this extra mode.
                """)
                TEXT_TO_INSERT = justificar_texto(TEXT_TO_INSERT)+'\n\n'
                parts.insert(-1,f"{TEXT_TO_INSERT}")
