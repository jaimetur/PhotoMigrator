...
from Core.GlobalVariables import SCRIPT_NAME_VERSION, SCRIPT_VERSION, ARGS, HELP_TEXTS, MSG_TAGS, TIMESTAMP, LOGGER, DIR_FOR_LOGS
...
def start_dashboard(migration_finished, SHARED_DATA, parallel=True, log_level=None):
    ...
    try:
        ...
        def build_log_panel():
            ...
            """
            title_logs_panel = f"📜 Logs Panel (Only last {logs_panel_height} rows shown. Complete log file at: '{DIR_FOR_LOGS}/{os.path.basename(log_file)}')"
            try:
                while True:
