...
def set_help_texts():
    ...
    HELP_TEXTS["merge-duplicates-albums"] = textwrap.dedent(f"""
    ...
    HELP_TEXTS["download-all"] = textwrap.dedent(f"""
    ...
    HELP_TEXTS["consolidate-albums-names"] = textwrap.dedent(f"""
            ...
            Each grouping algorithm can be disabled independently with '--no-try-equivalent-albums-grouping', '--no-try-date-prefix-albums-grouping', '--no-try-truncated-albums-grouping', or '--no-try-small-albums-grouping'. All are enabled by default.
