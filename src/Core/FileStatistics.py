    ...
    def process_block(file_paths, block_index, temporary_directory, extract_dates, step_name):
        ...
        for file_path in file_paths:
            ...
            if not extract_dates:
                ...
                norm_fp = Path(file_path).resolve().as_posix().lower()
                file_date = metadata_map.get(norm_fp)

            dates_by_path[norm_fp] = file_date

            if media_category:
                counters[media_category]['total'] += 1
                # If the oldest_date found is after reference, don't count as with_date
                if file_date and file_date < reference_timestamp:
                    counters[media_category]['with_date'] += 1
                else:
                    counters[media_category]['without_date'] += 1
