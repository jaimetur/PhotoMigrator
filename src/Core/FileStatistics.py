...
# ---------------------------------------------------------------------------------------------------------------------------
# THIS MODULE IS DEPRECATED. IT HAS BEEN REPLACED BY CLASS FolderAnalyzer WITHIN FolderAnalyzer.py MODULE
# ---------------------------------------------------------------------------------------------------------------------------
...
def count_files_and_extract_dates(input_folder, max_files=None, exclude_ext=None, include_ext=None, output_file="dates_metadata.json", extract_dates=True, step_name='', log_level=None):
    ...
    Args:
        input_folder (str): Folder to scan recursively.
        max_files (int|None): Optional cap on the number of files collected from disk.
        exclude_ext (list[str]|None): Extensions to exclude (with or without leading dot).
        include_ext (list[str]|None): If provided, only these extensions are included (with or without leading dot).
        output_file (str): Output JSON filename (if no extension is provided, .json is appended).
        extract_dates (bool): If True, extract dates via exiftool (fallback to PIL) and produce JSON output.
    ...
    Returns:
        tuple:
            - final_counters (dict): File statistics counters.
            - exif_data_by_path (dict): Dict indexed by normalized path -> exif metadata entry (only if extract_dates=True).
            - output_filepath (str): Final JSON output path (empty string if not produced).
    """
    ...
    def merge_counters(list_of_counter_dicts):
        """
        Merge per-block counters into a single counters dictionary.

        Args:
            list_of_counter_dicts (list[dict]): List of counters returned by process_block().

        Returns:
            dict: A merged counters dictionary.
        """
    ...
    def merge_json_files(chunk_files, final_output_path):
        """
        Merge JSON chunk files generated by each block into a single JSON file.

        Each chunk file is expected to be a JSON list. All lists are concatenated and written
        to final_output_path. Chunk files are removed after being merged.

        Args:
            chunk_files (list[str]): Paths to chunk JSON files.
            final_output_path (str): Path to write the merged JSON.
    ...
    def process_block(file_paths, block_index, temporary_directory, extract_dates, step_name):
        """
        Process a file block: extract dates (optional) and count files with/without a valid date.

        This function:
            - Creates a unique chunk JSON file path in the temporary directory
            - If extract_dates=True, tries exiftool first, and falls back to PIL if needed
            - Stores the selected oldest valid date per file in metadata_map
            - Counts per-media category (photos/videos) based on whether effective_date is valid

        Args:
            file_paths (list[str]): List of file paths in this block.
            block_index (int): Zero-based block index.
            temporary_directory (str): Temporary directory path where chunk JSON is written.
            extract_dates (bool): Whether to extract dates and output metadata JSON.
            step_name (str): Optional prefix for log messages.

        Returns:
            tuple:
                - counters (dict): Counters for this block.
                - chunk_json_path (str): Path to the chunk JSON output for this block.
        """
        def get_unique_path(path):
            """
            Generate a unique filesystem path by appending " (n)" suffix if the path already exists.

            Args:
                path (str): Desired path.

            Returns:
                str: A unique path that does not exist yet.
            """
            base = Path(path)
            counter = 1
            while base.exists():
        ...
        # Path to chunk_json in temporary directory (this variable should exist even if extract_dates=False or fallback to PIL)
    ...
    def main():
        ...
        Execute the full workflow:
            1) Build include/exclude extension filters
            2) Walk the filesystem and collect paths (skipping hidden dirs, @eaDir, and broken symlinks)
            3) Compute global counters (types, sizes, symlinks, etc.)
            4) Split media files into fixed blocks and process in parallel threads
            5) Merge per-block JSON outputs (if enabled)
            6) Merge counters and compute final percentages
            7) Load merged JSON and return it as a dict indexed by normalized path

        Returns:
            tuple:
                - final_counters (dict)
                - exif_data_by_path (dict)
                - output_filepath (str)
        """
        with set_log_level(LOGGER, log_level):
            ...
            for file_path in all_file_paths:
                ...
                # Exclude symlinks from the total_bytes count
                if not is_symlink:
                    try:
            ...
            with TemporaryDirectory() as temp_dir:
                ...
                with ThreadPoolExecutor(max_workers=num_threads) as executor:
                    # Submit each block to the executor
                    future_to_block = {
            ...
            # Load the final combined JSON as a dict indexed by path to return it instead of merged_dates
            exif_data_by_path = {}
            if extract_dates:
                try:
