...
from Core.GlobalVariables import TIMESTAMP, FOLDERNAME_EXIFTOOL, LOGGER, FOLDERNAME_EXIFTOOL_OUTPUT, PHOTO_EXT, VIDEO_EXT, METADATA_EXT, SIDECAR_EXT
from Utils.DateUtils import normalize_datetime_utc, is_date_valid
from Utils.GeneralUtils import print_dict_pretty
from Utils.StandaloneUtils import get_exif_tool_path, custom_print
    ...
    def extract_dates(self, step_name='', log_level=None):
        ...
        with set_log_level(self.logger, log_level):
            ...
            for block_index, file_block in enumerate(file_blocks, 1):
                ...
                # --- Try ExifTool
                if Path(exif_tool_path).exists():
                    json_output = os.path.join(FOLDERNAME_EXIFTOOL_OUTPUT, f"{TIMESTAMP}_block_{block_index}.json")
                    command = [exif_tool_path, "-j", "-n", "-time:all", "-fast", "-fast2", "-s"] + file_block
                    try:
                        with open(json_output, "w", encoding="utf-8") as f:
                            run(command, stdout=f, stderr=open(os.devnull, 'w'), check=True)
                ...
                for entry in metadata_list:
                    ...
                    for tag in candidate_tags:
                        ...
                        if isinstance(raw, str):
                            try:
                                ...
                                if is_date_valid(dt, reference) and (not dt_final or dt < dt_final):
                                    dt_final = dt
                                    source = f"EXIF:{tag}"
                            except:
                                continue
                    ...
                    self.file_dates[file_path] = {
                        "SourceFile": file_path,
                        "TargetFile": file_path,
                        "SelectedDate": dt_final.isoformat() if dt_final else None,
...
if __name__ == "__main__":
    ...
    input_folder = r"r:\jaimetur\PhotoMigrator\data\LocalFolderFromTakeout"

    # Crear el analizador
    analyzer = FolderAnalyzer(folder_path=input_folder)
