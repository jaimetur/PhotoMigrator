...
def read_immich_config(config_file='Immich.config', show_info=True):
    ...
    try:
        ...
        LOGGER.info(f"WARNING: Configuration file '{config_file}' not found. Data will be requested interactively...")

    IMMICH_URL = CONFIG.get('IMMICH_URL', None)
    API_KEY    = CONFIG.get('API_KEY', None)
    USERNAME   = CONFIG.get('USERNAME', None)
    PASSWORD   = CONFIG.get('PASSWORD', None)
    # If any data is missing, prompt the user for it
    if not IMMICH_URL:
        CONFIG['IMMICH_URL'] = input("[PROMPT] Enter IMMICH_URL (e.g., http://192.168.1.100:2283): ")
        IMMICH_URL = CONFIG['IMMICH_URL']
    if not API_KEY:
        if not USERNAME:
            CONFIG['USERNAME'] = input("[PROMPT] Enter USERNAME (Immich email): ")
            USERNAME = CONFIG['USERNAME']
        if not PASSWORD:
            CONFIG['PASSWORD'] = input("[PROMPT] Enter PASSWORD: ")
            PASSWORD = CONFIG['PASSWORD']
    else:
        API_KEY_LOGIN = True
    if show_info:
        LOGGER.info(f"INFO: Connecting to Immich:")
        LOGGER.info(f"INFO: IMMICH_URL : {IMMICH_URL}")
        if API_KEY_LOGIN:
            masked_password = '*' * len(API_KEY)
            LOGGER.info(f"INFO: API_KEY : {masked_password}")
        else:
...
def login_immich():
    ...
    if len(HEADERS.keys())>0 and  (f"Bearer {SESSION_TOKEN}" or API_KEY in HEADERS.values()):
        return True
    # Ensure the configuration is read
    read_immich_config()
    # Create Base headers for subsequent requests
    # If detected API_KEY in Immich.config
    if API_KEY_LOGIN:
        HEADERS = {
        ...
        LOGGER.info(f"INFO: API_KEY found in Immichc.config for authentication.")
    # If not detected API_KEY in Immich.config
    else:
        ...
        LOGGER.info(f"INFO: Authentication with user/password successful. Bearer Token obtained.")
    return True
...
def list_all_assets():
    """
    Returns a list of ALL assets (photos, videos) of the user in Immich.
    Each item includes metadata such as { id, deviceAssetId, fileCreatedAt, exifInfo, etc. }
    """
    from LoggerConfig import LOGGER  # Import global LOGGER
    if not login_immich():
        return []
    ...
    try:
        response = requests.get(url, headers=HEADERS, verify=False)
        response.raise_for_status()
        data = response.json()  # List of assets
        return data
    except Exception as e:
        LOGGER.error(f"ERROR: Failed to retrieve asset list: {e}")
        return []
...
def immich_upload_albums(input_folder):
    ...
    for item in os.listdir(input_folder):
        ...
        if os.path.isdir(subpath):
            ...
            if not album_id:
                LOGGER.warning(f"WARNING: Could not create album for subfolder '{item}'.")
                albums_skipped += 1
                continue
            albums_created += 1
...
def immich_delete_empty_albums():
    ...
    for album in tqdm(albums, desc="Searching for empty albums", unit="album"):
...
def immich_delete_duplicates_albums():
    ...
    if not albums:
        return 0
    duplicates_map = {}
    for album in tqdm(albums, desc="Searching for duplicate albums", unit="album"):
        album_id = album.get("id")
        album_name = album.get("albumName")
        assets_count = album.get("assetCount")
...
def immich_download_albums(album_name_or_id='ALL', output_folder="DownloadedAlbums"):
    """
    ...
    for album in albums_to_download:
        ...
        for asset in tqdm(assets, desc=f"Downloading '{album_name}'", unit="photos"):
            aid = asset.get("id")
            if aid:
...
# -----------------------------------------------------------------------------
def immich_download_all(output_folder="ImmichDownload"):
    """
    ...
    LOGGER.info(f"INFO: Found {len(leftover_assets)} asset(s) not in any album.")
    for asset in tqdm(leftover_assets, desc="Downloading UNASSOCIATED assets", unit="photos"):
        aid = asset.get("id")
        if not aid:
            continue
