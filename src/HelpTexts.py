def set_help_texts():
    ...
    HELP_TEXTS["AUTOMATED-MIGRATION"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will do an AUTOMATED-MIGRATION pre_process, Downloading all your Assets (including Albums) from the <SOURCE> Cloud Service
    ...
    HELP_TEXTS["google-photos-takeout"] = textwrap.dedent(f"""
        ATTENTION!!!: This module will pre_process your <TAKEOUT_FOLDER> to fix metadata of all your assets and organize them according with the settings defined by user (above settings).
        """)
    ...
    HELP_TEXTS["synology-remove-empty-albums"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to your to your Synology Photos account and will delete all Empty Albums found in Synology Photos database.
        """)

    HELP_TEXTS["synology-remove-duplicates-albums"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to your to your Synology Photos account and will delete all Duplicates Albums found in Synology Photos database.
        """)

    HELP_TEXTS["synology-upload-folder"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to your to your Synology Photos account and will upload all Photos/Videos found within <FOLDER> (including subfolders, except 'Albums' subfolder).
    ...
    HELP_TEXTS["synology-upload-albums"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to your to your Synology Photos account and will create a new Album for each Subfolder found in <ALBUMS_FOLDER> and will include all Photos and Videos included in that Subfolder.
    ...
    HELP_TEXTS["synology-upload-all"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to your Synology Photos account and will Upload all Assets found in <INPUT_FOLDER> 
    ...
    HELP_TEXTS["synology-download-albums"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to Synology Photos and download those Album(s) whose name is in: '<ALBUMS_NAME>' 
    ...
    HELP_TEXTS["synology-download-all"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to Synology Photos and will download all the Albums Assets and also 
    ...
    HELP_TEXTS["immich-remove-empty-albums"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to your to your Immich Photos account and will delete all Empty Albums found in Immich Photos database.
        """)

    HELP_TEXTS["immich-remove-duplicates-albums"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to your to your Immich Photos account and will delete all Duplicates Albums found in Immich Photos database.
        """)

    HELP_TEXTS["immich-upload-folder"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to your to your Immich Photos account and will upload all Photos/Videos found within <INPUT_FOLDER> (including subfolders, except 'Albums' subfolder).
        """)

    HELP_TEXTS["immich-upload-albums"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to your to your Immich Photos account and will create a new Album for each Subfolder found in <ALBUMS_FOLDER> and will include all Photos and Videos included in that Subfolder.
        """)

    HELP_TEXTS["immich-upload-all"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to your to your Immich Photos account and will Upload all Assets found in <INPUT_FOLDER> 
    ...
    HELP_TEXTS["immich-download-albums"]  = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to Immich Photos and download those Album(s) whose name is in: '<ALBUMS_NAME>' 
        to the output folder: '<OUTPUT_FOLDER>'.  If the file already exists, it will be OVERWRITTEN!!!
    ...
    HELP_TEXTS["immich-download-all"]  = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will connect to Immich Photos and will download all the Albums Assets and also 
        Assets without Albums into the folder '<OUTPUT_FOLDER>'. If the file already exists, it will be OVERWRITTEN!!!
    ...
    HELP_TEXTS["immich-remove-orphan-assets"]  = textwrap.dedent(f"""
        ATTENTION!!!: In this pre_process, the script will look for all Orphan Assets in Immich Database and will delete them. 
    ...
    HELP_TEXTS["find-duplicates"]  = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will pre_process all Duplicates files found in <DUPLICATES_FOLDER> and will apply the given action.
        ...
        Possible duplicates-action are:
            ...
            - move   : This action could be dangerous but is easily reversible if you find that any duplicated file have been moved to Duplicates folder and you want to restore it later
                       You can easily restore it using option -pd, --pre_process-duplicates
    ...
    HELP_TEXTS["pre_process-duplicates"]  = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will pre_process all Duplicates files found with option '-findDup, --find-duplicates <ACTION> <DUPLICATES_FOLDER>' 
    ...
    HELP_TEXTS["fix-symlinks-broken"] = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will look for all Symbolic Links broken in <FOLDER_TO_FIX> and will try to find the destination file within the same folder.
        """)

    HELP_TEXTS["rename-folders-content-based"]  = textwrap.dedent(f"""
        ATTENTION!!!: This pre_process will clean each Subfolder found in <ALBUMS_FOLDER> with an homogeneous name starting with album year followed by a cleaned subfolder name without underscores nor middle dashes.
