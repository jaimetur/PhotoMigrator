...
# Entrar en modo interactivo si se llama con "bash"
if [[ "$1" == "bash" ]]; then
...
if [ ! -r "$DOCKER_CONF_FILE" ]; then
    echo "❌ docker.conf not found or not readable at $DOCKER_CONF_FILE"
...
if [ ! -r "$CONFIG_FILE" ]; then
    echo "❌ Config.ini not found or not readable at $CONFIG_FILE"
    if [ -w "$CONFIG_DIR" ]; then
