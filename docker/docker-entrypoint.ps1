...
# Validar Config.ini
Write-Host "Looking for: <CONFIGURATION_FILE>"
if (-Not (Test-Path $CONFIG_FILE)) {
    Write-Host "❌ <CONFIGURATION_FILE> not found in the current folder."
    Write-Host "Creating a default configuration file..."
    Copy-Item $DEFAULT_CONFIG $CONFIG_FILE
    Write-Host "Please edit <CONFIGURATION_FILE> with your settings and run the script again."
    exit 1
}
