...
- **Explanation:**
  ...
  - When both behaviors are active, PhotoMigrator prefers the clean keeper name, merges the assets from numbered/underscored variants into that keeper, and reuses it for the incoming upload.
...
- **Example of use:**
  ...
  ./PhotoMigrator.bin --client=google-photos --upload-albums ./My_Albums_Folder --prefer-canonical-album-names --consolidate-similar-albums
  ```
  ...
  If no equivalent album already exists in the target and you upload `New_Album 1`, the same flag creates `New Album` directly instead of preserving the duplicate-like suffix.
...
- **Explanation:**
  ...
  - When both behaviors are active, PhotoMigrator prefers the clean keeper name and merges the assets from redundant variants into that keeper before continuing with the incoming upload.
...
- **Example of use:**
  ...
  ./PhotoMigrator.bin --client=google-photos --upload-all ./MyLibrary --prefer-canonical-album-names --consolidate-similar-albums
  ```
...
- **Explanation:**
  ...
  - Albums with up to three assets can also be consolidated into a larger similarly named keeper only when every capture date from the small album is present in the keeper.
