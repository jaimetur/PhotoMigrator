...
- **Windows (PowerShell)**
  ```bash
  curl.exe -L -o PhotoMigrator_v4.1.0_docker.zip https://github.com/jaimetur/PhotoMigrator/releases/download/v4.1.0/PhotoMigrator_v4.1.0_docker.zip
  ```
...
- **Linux/macOS**
  ```bash
  7z x PhotoMigrator_v4.1.0_docker.zip
  cd PhotoMigrator/docker-cli
  ```

- **Windows (PowerShell)**
  ```bash
  powershell -Command "Expand-Archive -Path PhotoMigrator_v4.1.0_docker.zip -DestinationPath ./"
  cd PhotoMigrator\docker-cli
  ```
...
- Specific version
  ```bash
  docker pull jaimetur/photomigrator-linux:4.1.0
  ```
