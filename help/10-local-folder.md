...
`--consolidate-albums-names` applies the same album-family rules used by the cloud services without uploading media: canonical equivalent names, compatible `YYYY`, `YYYY-MM`, and `YYYY-MM-DD` prefixes, guarded end-truncated names, and small albums with up to the configurable `--small-album-max-assets` limit (default `3`) whose complete capture-date range fits in a larger similarly named keeper. The limit is used only when `--try-small-albums-grouping` is enabled. Date components can be separated by dots, underscores, hyphens, long dashes, or spaces. A specific date remains the keeper only when at least 95% of its album assets are inside that date range; otherwise the compatible broader date is retained.

Truncated names require at least two distinct shared title words and the same dominant asset year. Bare dates do not qualify as titles. Plain names are kept separate from `Shared`, `Share`, `Public`, `Público`, `X`, and partial forms of those suffixes; if only `Videos` differs, the non-`Videos` name is the keeper. A terminal date already included in the leading date or year range is redundant, so the name without it is retained. The default preview table shows the group, match rule, keeper, candidates to merge, and keeper-decision comments; confirmation is requested by default.

All four detectors are enabled independently by default: `--try-equivalent-albums-grouping`, `--try-date-prefix-albums-grouping`, `--try-truncated-albums-grouping`, and `--try-small-albums-grouping`. Their `--no-...` variants disable only the selected rule.
