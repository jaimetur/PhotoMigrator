...
`--consolidate-albums-names` applies the same album-family rules used by the cloud services without uploading media: canonical equivalent names, compatible `YYYY`, `YYYY-MM`, and `YYYY-MM-DD` prefixes, guarded end-truncated names, and small albums with up to three assets whose complete capture-date range fits in a larger similarly named keeper. Date components can be separated by dots, underscores, hyphens, long dashes, or spaces. A specific date remains the keeper only when at least 95% of its album assets are inside that date range; otherwise the compatible broader date is retained.
