...
When the destination is Immich, **`-iPeople, --import-people=true`** imports people labels from a Google Takeout source or a local source folder that contains `takeout_people_metadata.json`; it is enabled by default and can be disabled with `false`. A raw local folder with Google JSON sidecars is also supported. A raw Google Takeout source is processed first and produces this map automatically when **`-gPeople, --google-process-people=true`** (the default); set it to `false` to skip Step 4.1 and not create the map. The Takeout processor writes the map during Step 4.1 and rewrites it during Final Cleaning if GPTH removed it. For repeated filenames, PhotoMigrator compares only equivalent date sources: EXIF with `photoTakenTime`, then filesystem `mtime` with `modificationTime`. `created_at` is preserved for reference but is not used for matching because GPTH does not preserve a portable filesystem creation time. Album copies with the same capture time are consolidated before this comparison; genuinely equal results merge their people labels and are logged.
