...
| `-iPeople`,<br>`--import-people` `<bool>`             | Immich Upload All, Upload Albums, or Immich-target Automatic Migration only: import Takeout people labels (default: `true`)                                                                                                                                                                                     |
| `-cStacks`,<br>`--create-stacks` `<bool>`             | Immich Upload All, Upload Albums, or Immich-target Automatic Migration only: create stacks for burst-like photos (default: `true`)                                                                                                                                                                              |
...
| `-consSimAlb`,<br>`--consolidate-similar-albums`      | Reuse equivalent-name, compatible-date, guarded truncated-name, and small-album date-match families during cloud uploads and Automatic Migration                                                                                                                                                                |
| `-consAlbNames`,<br>`--consolidate-albums-names`      | Consolidate the same guarded album families directly in the target service; preview reports keeper, candidates, rule, and asset-date use                                                                                                                                                                        |
| `-tryEqAlb`,<br>`--try-equivalent-albums-grouping`    | Try canonical/equivalent-name consolidation in Consolidate Albums Names (default: `true`)                                                                                                                                                                                                                       |
| `-tryDateAlb`,<br>`--try-date-prefix-albums-grouping` | Try compatible date-prefix consolidation in Consolidate Albums Names (default: `true`)                                                                                                                                                                                                                          |
| `-tryTruncAlb`,<br>`--try-truncated-albums-grouping`  | Try guarded truncated-name consolidation in Consolidate Albums Names (default: `true`)                                                                                                                                                                                                                          |
| `-trySmallAlb`,<br>`--try-small-albums-grouping`      | Try small-album date-match consolidation in Consolidate Albums Names (default: `true`)                                                                                                                                                                                                                          |
| `-smallAlbMax`,<br>`--small-album-max-assets`         | Maximum assets for an album to qualify as small (default: `3`; used only with small-album grouping enabled)                                                                                                                                                                                                     |
...
| `-prevAlbAct`,<br>`--preview-album-actions`           | Preview rename/remove/consolidation matches and confirm (default: `true`; disable with `--no-preview-album-actions`)                                                                                                                                                                                            |
...
| `-immichDupAlgo`,<br>`--dup-immich-native-algorithm`  | Immich only: similarity detection; disable for exact filename/size Takeout re-upload checks (default: `true`)                                                                                                                                                                                                   |
| `-immichDupDel`,<br>`--dup-immich-native-deletion`    | Immich only: available only with native detection plus `better-quality`, and enabled by default only for that combination. While enabled, `better-quality` is the only allowed keeper. `true` uses Immich's Alpha resolver (albums, favorites, rating, descriptions, visibility, matching location, tags; moves to trash). All other keeper strategies use PhotoMigrator's guarded merge and trash redundant native-group assets only after keeper verification. |
| `-mergeDupLoc`,<br>`--merge-duplicate-locations`      | Immich manual duplicate merge only. Disabled by default to preserve keeper GPS. When enabled, copies GPS only if the keeper has no location and at least two redundant assets have the same complete coordinate pair. |
| `-dupKeeper`,<br>`--dup-asset-keeper`                 | Keeper: `more-people/tags-then-better-quality`, `more-people/tags-then-oldest`, `more-people/tags-then-newest`, `better-quality`, `oldest`, or `newest`. Immich defaults to `more-people/tags-then-better-quality` with native detection and `more-people/tags-then-oldest` without it.                                               |
