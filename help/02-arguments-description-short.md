...
| `-immichDupDel`,<br>`--dup-immich-native-deletion`    | Immich only: `true` uses Immich's Alpha resolver (albums, favorites, rating, descriptions, visibility, matching location, tags; moves to trash); `false` uses PhotoMigrator's guarded merge (+ capture date and stacks; permanent delete). Unavailable with native detection off (default: `true` when enabled) |
| `-dupKeeper`,<br>`--dup-asset-keeper`                 | Keeper: `more-people/tags-then-better-quality`, `more-people/tags-then-oldest`, `more-people/tags-then-newest`, `better-quality`, `oldest`, or `newest`. Immich defaults to `better-quality` with native detection and `more-people/tags-then-newest` without it.                                                                                                             |
