...
| `-immichDupDel`,<br>`--dup-immich-native-deletion`    | Immich only: available only with native detection plus `better-quality`, and enabled by default only for that combination. `true` uses Immich's Alpha resolver (albums, favorites, rating, descriptions, visibility, matching location, tags; moves to trash). All other keeper strategies use PhotoMigrator's guarded merge and trash redundant native-group assets only after keeper verification. |
