...
| `-useImmichDupDet`,<br>`--use-immich-duplicates-detection` | `<true\|false>`                          |       boolean     | `(default: true)`                            | Immich `Remove Duplicate Assets` only. Native detection compares asset similarity rather than filename or size. When disabled, PhotoMigrator groups exact filename-and-size matches, useful when the same processed Takeout was uploaded on different dates and an EXIF tag difference prevented Immich from rejecting the later upload.                                                                                                                                                                                                                                                                  |
