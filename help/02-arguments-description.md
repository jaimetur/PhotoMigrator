...
| `-reuseSimAlb`,<br>`--reuse-similar-existing-albums` |                                                 |       flag        |                           | Reuses equivalent album families (for example `Album`, `Album_1`, `Album (2)`, `New_Album`, `New Album`, `New_Album 1`) during cloud uploads and Automatic Migration. It also normalizes newly created destination album names to the preferred clean keeper form even when the target does not already contain a similar variant. Supported cloud targets also consolidate redundant variants into that keeper. `Immich`, `Synology`, and `NextCloud` then remove the redundant albums; `Google Photos` keeps them because the public API cannot delete albums. |
