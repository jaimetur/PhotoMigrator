...
| `-rDupAst`,<br>`--remove-duplicates-assets`        |                                                 |       flag        |                                              | Removes duplicate physical assets with the same exact filename and file size. Requires a cloud `--client` and `--duplicate-asset-keeper`. Google Photos reports its public API deletion limitation and makes no changes.                                                                                                                                                                                                 |
| `-dupKeeper`,<br>`--duplicate-asset-keeper`        | `<KEEPER>`                                      |      string       | `oldest`,<br>`newest`<br>`(default: newest)` | Required module selector for `--remove-duplicates-assets`. Chooses the retained asset according to the timestamp exposed by the selected cloud backend.                                                                                                                                                                                                                                                                  |
