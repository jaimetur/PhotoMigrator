...
| `-lPhotosFolder`,<br>`--local-photos-folder` | `<LOCAL_PHOTOS_FOLDER>` |    path     |                                                 `existing path`                                                 | Managed library root required with `--client=local-photos-folder`. Its physical assets are stored in `No_Albums` and album membership is represented under `Albums`. |
| `-client`,<br>`--client`                     | `<CLIENT>`              |   string    | `google-takeout`, <br>`google-photos`, <br>`synology`, <br>`immich`, <br>`nextcloud`, <br>`local-photos-folder` | Specifies the service or managed local library to interact with.                                                                                                     |
| `-id`,<br>`--account-id`                     | `<ID>`                  |     int     |                                        `1`, `2`, `3` <br>`(default: 1)`                                         | ID of the configured cloud account in `Config.ini`; unused by Local Photos Folder.                                                                                   |
...
| `-createdFrom`,<br>`--created-from`          | `<DATE>`                |    date     |                                       `YYYY`, `YYYY-MM`, or `YYYY-MM-DD`                                        | For Remove Albums, keeps only albums created on or after the supplied date.                                                                                          |
| `-createdTo`,<br>`--created-to`              | `<DATE>`                |    date     |                                       `YYYY`, `YYYY-MM`, or `YYYY-MM-DD`                                        | For Remove Albums, keeps only albums created on or before the supplied date.                                                                                         |
...
| `-pushMaxMB`,<br>`--push-asset-max-size-mb`                  | `<MEGABYTES>` |      integer      |                          `0` or greater<br>`(default: 0)`                           | Maximum asset size eligible for Automatic Migration push retries. `0` removes the size limit; retry count and delay remain in effect. |
| `-immichUploadTimeout`,<br>`--immich-upload-timeout-seconds` | `<SECONDS>`   |      integer      |                        greater than `0`<br>`(default: 900)`                         | Immich upload read timeout for Upload All, Upload Albums, and Automatic Migration when Immich is the target.                          |
...
To use the following features, use `--client=[synology, immich, nextcloud, google-photos, local-photos-folder]`. Local Photos Folder additionally requires `--local-photos-folder <LOCAL_PHOTOS_FOLDER>` as its managed library root.
Cloud clients can optionally use `--id=[1-3]` to select an account defined in `Config.ini`. `--account-id` is not used by Local Photos Folder.
...
| `-consSimAlb`,<br>`--consolidate-similar-albums`      |                                                 |       flag        |                                                                                                                                                       | Reuses and consolidates equivalent album families during cloud uploads and Automatic Migration. In addition to canonical names, it supports compatible `YYYY`/`YYYY-MM`/`YYYY-MM-DD` prefixes (specific dates require at least 95% asset-date coverage), guarded end truncation (two shared words and the same dominant asset year), and small albums whose complete capture-date range fits in a larger similarly named keeper. Supported cloud targets merge redundant variants into the preferred keeper. `Immich`, `Synology`, and `NextCloud` then remove redundant albums; `Google Photos` keeps them because its API cannot delete albums. |
| `-consAlbNames`,<br>`--consolidate-albums-names`      |                                                 |       flag        |                                                                                                                                                       | Consolidates the same guarded equivalent-name, date-prefix, end-truncation, and small-album capture-date-match families directly in the target service without uploading new assets. Its preview table shows the group, match rule, keeper, candidates, and comments explaining the applied keeper decision. `Immich`, `Synology`, and `NextCloud` remove redundant albums afterwards; `Google Photos` keeps them because its API cannot delete albums.                                                                                                                                                                                           |
| `-tryEqAlb`,<br>`--try-equivalent-albums-grouping`    |                                                 |      boolean      |                                                                   `(default: true)`                                                                   | Enables canonical/equivalent-name grouping for Consolidate Albums Names. Use `--no-try-equivalent-albums-grouping` to disable it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `-tryDateAlb`,<br>`--try-date-prefix-albums-grouping` |                                                 |      boolean      |                                                                   `(default: true)`                                                                   | Enables compatible date-prefix grouping for Consolidate Albums Names. Use `--no-try-date-prefix-albums-grouping` to disable it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `-tryTruncAlb`,<br>`--try-truncated-albums-grouping`  |                                                 |      boolean      |                                                                   `(default: true)`                                                                   | Enables guarded truncated-name grouping for Consolidate Albums Names. Use `--no-try-truncated-albums-grouping` to disable it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `-trySmallAlb`,<br>`--try-small-albums-grouping`      |                                                 |      boolean      |                                                                   `(default: true)`                                                                   | Enables small-album capture-date-match grouping for Consolidate Albums Names. Use `--no-try-small-albums-grouping` to disable it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `-smallAlbMax`,<br>`--small-album-max-assets`         | `<COUNT>`                                       |      integer      |                                                                    `(default: 3)`                                                                     | Sets the maximum number of assets that qualifies an album as small for Consolidate Albums Names. This value is used only while `--try-small-albums-grouping` is enabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `-createdFrom`,<br>`--created-from`                   | `<DATE>`                                        |       date        |                                                          `YYYY`, `YYYY-MM`, or `YYYY-MM-DD`                                                           | Optional inclusive creation-date lower bound for Remove Albums.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `-createdTo`,<br>`--created-to`                       | `<DATE>`                                        |       date        |                                                          `YYYY`, `YYYY-MM`, or `YYYY-MM-DD`                                                           | Optional inclusive creation-date upper bound for Remove Albums.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `-rDupAst`,<br>`--remove-duplicates-assets`           |                                                 |       flag        |                                                                                                                                                       | Removes duplicate physical assets. Immich uses native visual duplicate groups by default; other cloud services and Local Photos Folder group exact filename-and-size matches. Requires `--client` and `--dup-asset-keeper`. Google Photos reports its public API deletion limitation and makes no changes.                                                                                                                                                                                                                                                                                                                                        |
...
| `-immichDupDel`,<br>`--dup-immich-native-deletion`    | `<true\|false>`                                 |      boolean      |                                                 `(available with native detection + better-quality)`                                                  | Immich `Remove Duplicates Assets` only. `true` uses Immich's Alpha resolver only when `--dup-immich-native-algorithm=true` and `--dup-asset-keeper=better-quality`; while enabled, `better-quality` is the only accepted keeper. Every other keeper strategy uses PhotoMigrator's guarded manual merge: album memberships, tags, favorite state, rating, descriptions, visibility, matching locations, missing capture date, and stacks are merged; the selected keeper and merged albums/tags are then verified before redundant native-group assets are moved to trash.                                                                         |
| `-mergeDupLoc`,<br>`--merge-duplicate-locations`      | `<true\|false>`                                 |      boolean      |                                                                   `(default: false)`                                                                  | Immich `Remove Duplicates Assets` manual-resolution only. By default, PhotoMigrator preserves the selected keeper location and never copies GPS coordinates from redundant assets. When enabled, it transfers GPS only if the keeper has no location and at least two redundant assets have the same complete latitude/longitude pair. It is ignored when Immich native deletion is enabled. |
| `-dupKeeper`,<br>`--dup-asset-keeper`                 | `<KEEPER>`                                      |      string       | `more-people/tags-then-better-quality`,<br>`more-people/tags-then-oldest`,<br>`more-people/tags-then-newest`,<br>`better-quality`, `oldest`, `newest` | Required module selector for `--remove-duplicates-assets`. The three `more-people/tags-*` strategies retain the asset with the largest distinct people count, then tag count, then use the named tie breaker. `more-people/tags-then-better-quality` is available only with Immich native detection. Immich defaults to `more-people/tags-then-better-quality` with native detection and `more-people/tags-then-oldest` without it.                                                                                                                                                                                                               |
