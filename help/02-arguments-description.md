...
| `-consSimAlb`,<br>`--consolidate-similar-albums`      |                                                 |       flag        |                                                                                                                                                       | Reuses and consolidates equivalent album families during cloud uploads and Automatic Migration. In addition to canonical names, it supports compatible `YYYY`/`YYYY-MM`/`YYYY-MM-DD` prefixes (specific dates require at least 95% asset-date coverage), guarded end truncation (two shared words and the same dominant asset year), and albums with up to three assets whose capture dates are all present in a larger similarly named keeper. Supported cloud targets merge redundant variants into the preferred keeper. `Immich`, `Synology`, and `NextCloud` then remove redundant albums; `Google Photos` keeps them because its API cannot delete albums. |
