...
| `-immichDupDel`,<br>`--immich-duplicates-deletion`   | `<true\|false>`                                 |      boolean      |                                `(default: false)`                                | Immich `Remove Duplicate Assets` only. When enabled, PhotoMigrator sends the selected keeper and redundant assets to Immich's native duplicate resolver, which merges Immich-supported metadata and moves redundant assets to trash. This endpoint is Alpha. When disabled, PhotoMigrator uses its guarded manual metadata-merge and permanent-deletion flow.                                                            |
