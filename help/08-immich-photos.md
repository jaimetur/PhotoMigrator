...
Immich `Upload Albums` and `Upload All` support `--import-people` when the selected input folder contains `takeout_people_metadata.json`, generated by the Google Takeout processor. PhotoMigrator loads the asset-name, person-label, and three-date map, resolves duplicate filenames by the nearest EXIF/filesystem-to-Takeout date distance, then mirrors immich-go's people-tag behavior by attaching hierarchical Immich tags named `people/<name>` to each matching asset. Equal nearest candidates merge their labels and are logged.
...
- **Usage:**
  ...
  - Optional _**`--immich-duplicates-deletion=true`**_ delegates metadata merging and redundant-asset removal to Immich's native duplicate resolver. It is disabled by default because that endpoint is Alpha; its redundant assets are moved to trash rather than permanently deleted.
