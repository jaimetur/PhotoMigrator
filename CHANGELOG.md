...
#### 🌟 New Features:
  ...
  - Added an `Estimated Time` indicator to the `Automatic Migration` live dashboard in both the terminal UI and the Web Interface. The estimate is computed from the current elapsed time and the number of already processed assets (`pushed`, `failed`, and `duplicated`) so long-running migrations now show an approximate remaining duration while the job is in progress.
