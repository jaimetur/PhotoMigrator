...
#### 🚀 Enhancements:
  ...
  - Optimized the Immich `Remove Duplicate Assets` inventory scan with a lightweight `/api/search/statistics` preflight that obtains the exact user-visible matching-asset total, followed by explicit 1,000-asset metadata pages and one determinate progress bar. The initial scan now states clearly that a full-library analysis can take several minutes on large libraries instead of appearing stalled after authentication; servers that do not support the statistics endpoint fall back safely to indeterminate progress.
