...
#### 🐛 Bug fixes:
  ...
  - Fixed the temporary 50-candidate Immich duplicate-review cap for exact filename-and-size detection. Oversized groups are now truncated to the remaining preview capacity instead of bypassing the cap when `--immich-duplicates-algorithm=false`.
