...
#### 🚀 Enhancements:
  ...
  - Optimized Google Takeout album-entry reconciliation. Step 5.5, now named `Reconcile Album Entries with ALL_PHOTOS`, indexes an album once while processing all of its sidecars and updates that index in memory after creating entries, avoiding repeated recursive scans of the same album without weakening filename, date-folder, or byte-for-byte validation.
  - Clarified Google Takeout Step 5.5 reconciliation reporting. Successful physical-copy-to-shortcut replacements are now `INFO` rather than review warnings; repeated sidecars no longer repeat the same physical-entry warning; per-album totals now count actual physical album entries instead of JSON sidecars; and the phase ends with a multi-line summary separating evaluated sidecars, existing mappings, missing entries, recovered entries, repaired shortcuts, retained review items, unresolved entries, and affected albums.
