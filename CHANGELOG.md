...
#### 🐛 Bug fixes:
  ...
  - Fixed the Rich `Automatic Migration` Live Dashboard finalization and layout: GUI-launched dashboards now close automatically after their final refresh so completion is reported back to the GUI and its `Run` / `Stop` controls reset correctly. The Info, Pull, and Push panel dividers are now rendered as full-width Rich rules instead of split table-cell fragments.
