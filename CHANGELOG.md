...
#### 🚀 Enhancements:
  ...
  - Updated all `Automatic Migration` live dashboards (Web, CLI, TUI, and GUI) so `Delayed Retry Queue` and `Album Assoc Queue` use the total number of migration assets as their progress maximum. `Album Assoc Queue` now appears directly below `Delayed Retry Queue`.
  - Reworked `Automatic Migration` Push dashboards across Web, CLI, TUI, and GUI. The `Pushed Assets`, `Pushed Photos`, and `Pushed Videos` bars now track physical assets admitted to the push pipeline, while `New`, `Duplicates`, and final `Failed` outcomes are displayed separately for each media type. Added cumulative `Delayed Retries` and `Delayed Recovered` counters; album-association follow-up failures remain excluded from hard push failures.
  - Grouped each Push dashboard outcome summary directly below its corresponding Assets, Photos, Videos, or Albums progress bar as `Total <type> (New / Duplicates / Failed)`, and renamed the backlog indicator to `Delayed Retry Queue`.
