...
#### 🐛 Bug fixes:
  ...
  - Fixed Web Interface command-preview state collection for checkbox flags so optional boolean flags such as `--google-keep-takeout-folder`, `--google-remove-duplicates-files`, `--google-rename-albums-folders`, and `--google-skip-extras-files` now update the generated command reliably when toggled instead of potentially keeping a stale enabled value in the collected form payload. The preview payload builder now merges the latest in-memory UI state with the currently visible form values before calling `/api/preview`, which hardens `Google Takeout` checkbox handling even when a specific visible control is missed by the DOM collector. The shared/UI command builders were also covered with explicit tests for these `Google Takeout` flag emissions.
