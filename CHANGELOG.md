...
#### 🌟 New Features:
  ...
  - **Local Folder Management:** Added a Local Folder tab after NextCloud Photos. It provides the common cloud-library modules against a managed on-disk library through `--client=local-folder --local-folder <LOCAL_FOLDER>`, with physical assets stored under `No_Albums` and album membership represented without duplicating media files.
