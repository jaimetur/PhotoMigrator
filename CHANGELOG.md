...
#### 🚀 Enhancements:
  ...
  - Optimized the Web Interface `Execution Output` panel for long-running jobs with very large logs. The backend now exposes the already-compacted logical log lines separately from the raw output text, and the browser virtualizes the log rendering so only the visible lines plus a surrounding buffer are painted while scrolling. This avoids reparsing and repainting thousands of off-screen lines on every refresh during large `Automatic Migration` runs. The polling API now ships only a small recent window of compact log lines while the browser keeps an incremental local scroll buffer, which reduces JSON payload size and helps avoid growing lag between the physical log file and the live web log panel when the run output reaches several MB. (Issue #1175).
  ...
  - Simplified the Web Interface `Execution Output` panel again so it renders the full compact in-memory log buffer for the current job instead of stitching paginated history batches in the browser. Dashboard counters still come from structured snapshots, so the live panel keeps the recovered performance without the scroll glitches, blank gaps, and split progress lines introduced by incremental history merging.
  ...
  - Extended the Web Interface `Execution Output` panel with on-demand paginated history loading from the persisted per-job log file. Live polling still only transfers a small recent tail to keep `Automatic Migration` responsive, while scrolling upward now fetches older log blocks incrementally instead of re-downloading or reparsing the full log on every refresh. (Issue #1175).
...
#### 🐛 Bug fixes:
  ...
  - Fixed the Web Interface `Execution Output` progress rendering so active progress-bar updates no longer force a full repaint of the visible log window on every poll. When the visible virtualized range stays the same, the browser now patches only the individual log lines whose rendered content actually changed, which avoids the distracting up/down jump effect previously caused by repeatedly repainting a progress line together with the lines that follow it.
  ...
  - Formalized the shared photo-client public contract used by cross-backend features such as `Automatic Migration`. A new base client contract now defines the common polymorphic operations (`create_album`, album listing, album asset listing, add-to-album, push, pull, etc.), and the concrete `Local Folder`, `Synology Photos`, `Immich Photos`, `NextCloud Photos`, and `Google Photos` clients were updated to expose aligned public method signatures for those operations. Optional context parameters that only matter for specific backends (for example Synology album context) are now accepted consistently as no-ops by the other clients, and a dedicated signature-alignment test was added to catch future interface drift before it becomes a runtime migration failure.
