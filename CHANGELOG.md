...
#### 🚀 Enhancements:
  ...
  - Expanded the startup log for every feature with the selected feature and its required flags before `Global Settings`, followed by an `Optional Flags Used` section that lists only flags explicitly supplied by the caller. Modular cloud and Other Features runs now also identify their selected module. Configurable flag values were removed from `Global Settings`, leaving only hardcoded or runtime-derived settings there.
