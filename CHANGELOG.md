...
#### 🚀 Enhancements:
  ...
  - Reduced Automatic Migration log noise by suppressing per-album `Album Pushed` reconciliation lines. When album consolidation or canonical naming is enabled, each completed consolidation/canonicalization now logs its source and destination names together with the original and final destination asset counts.
