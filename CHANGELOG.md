...
#### 🚀 Enhancements:
  - Hardened Google Takeout orphan album-sidecar recovery. A JSON-only album entry is now recreated only when its exact filename resolves to one unique candidate in the JSON capture year; cross-year, normalized-name, missing-year, and same-year ambiguous matches remain unresolved instead of being added to the wrong album. The processor writes `orphan_album_asset_recovery_manifest.json` with the source sidecar, result, and candidates for later audit. Existing valid album shortcuts are treated as resolved without being counted as orphans or generating year-mismatch warnings; only actual missing entries and shortcut-strategy inconsistencies, such as a physical copy or an external/broken link where a shortcut is expected, require review.
