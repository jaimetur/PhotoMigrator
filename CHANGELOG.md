...
#### 🚀 Enhancements:
  ...
  - Refined Automatic Migration per-asset `Album Queue` logging for dynamically populated staging folders. Push workers now atomically reserve the claimed asset and snapshot only the on-disk unclaimed files for that album before upload, avoiding stale repeated pending counts without changing transfer counters.
