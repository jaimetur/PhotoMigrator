...
#### 🚀 Enhancements:
  ...
  - Enhanced Immich `Remove Duplicate Assets` to preserve assigned people when deleting a duplicate group. The module now transfers faces only when every asset has the same Immich checksum, retrieves their geometry, compares normalized person-and-bounding-box records, and creates only faces missing from the keeper before deletion. Groups containing unassigned, malformed, incomplete, unreadable, or non-identical face data remain unchanged to avoid losing or misassigning face metadata.
  - Added Immich-native duplicate detection to `Remove Duplicate Assets` through `--immich-duplicates-algorithm=true|false` (default: enabled). The module now uses Immich's visually similar duplicate groups by default, while still previewing, confirming, transferring supported metadata, and deleting through PhotoMigrator's guarded flow. `--duplicate-asset-keeper` adds the `better-quality` strategy, selected by default with native detection and based on Immich's suggested quality keeper; `oldest` and `newest` remain available.
  ...
  - Improved Immich duplicate-review labels by resolving album, tag, and person UUIDs through cached catalog maps, retaining UUIDs only when Immich cannot resolve a name.
  - Optimized native Immich duplicate deletion by loading each candidate's complete asset metadata only once for the confirmation preview while omitting its separate album-membership lookup. Immich does not expose memberships in bulk and its server-side resolver preserves them, so the native flow avoids one expensive API request per duplicate candidate; the guarded manual deletion path still loads album memberships because it needs those records to perform its own merge.
  ...
  - Removed the redundant native Immich duplicate-review log notice about omitted album-membership lookups; the metadata progress bar remains the only progress message for that phase.
  - Raised the bounded Immich duplicate-review metadata hydration pool from 24 to 100 asset workers. The manual merge now also matches Immich's location rule: coordinates are copied only when every geotagged asset in the group has the same latitude and longitude.
  - Restored complete native Immich duplicate-review metadata: PhotoMigrator now reloads the paginated person ID-to-name map and every candidate's album memberships so the confirmation preview shows person and album names rather than unresolved UUIDs.
