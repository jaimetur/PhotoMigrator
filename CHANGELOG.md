...
#### 🐛 Bug fixes:
  ...
  - Restored the detailed Immich `Remove Duplicates Assets` review table after the service-client cleanup delegation reduced it to one compact line per group. The preview again compares the selected keeper and every removal candidate by ID, size, upload time, and available metadata before confirmation, without changing whether native Immich or exact filename-and-size detection is selected.
