...
#### 🚨 Breaking Changes:
  ...
  - Renamed duplicate-asset arguments: `--immich-duplicates-algorithm` is now `--duplicates-immich-native-algorithm`, `--immich-duplicates-deletion` is now `--duplicates-immich-native-deletion`, and `--duplicate-asset-keeper` is now `--duplicates-asset-keeper`. The former long names are no longer accepted.

#### 🌟 New Features:
  - **Remove Duplicates Assets:** Added `--remove-duplicates-assets` to every cloud-service feature. Synology Photos, Immich Photos, and NextCloud Photos can remove duplicate physical assets using the required `--duplicates-asset-keeper` selector; Google Photos exposes the analysis and selection flow but clearly reports that its public Library API cannot delete media items. Immich additionally supports native visual duplicate detection and native metadata-preserving resolution, with PhotoMigrator's exact filename-and-size/manual-merge flow available as a fallback.
...
#### 🚀 Enhancements:
  ...
  - Renamed the Immich native duplicate-detection flag to `--duplicates-immich-native-algorithm`.
  ...
  - Linked the Immich duplicate-resolution controls across CLI, Web, TUI, and GUI: disabling `--duplicates-immich-native-algorithm` now disables `--duplicates-immich-native-deletion` and selects PhotoMigrator's manual flow; enabling native detection defaults the native-deletion control back to enabled.
  - Hardened the CLI dependency between the Immich duplicate flags: `--duplicates-immich-native-deletion` defaults to `true` with native detection, but an explicitly enabled native-deletion flag is rejected when `--duplicates-immich-native-algorithm=false`.
  - Prevented interactive Immich manual duplicate runs from emitting the disabled native-deletion option. With native detection off, Web, GUI, and TUI previews now omit `--duplicates-immich-native-deletion`; CLI validation also accepts an explicit `false` value while continuing to reject native deletion enabled without native detection.
  ...
  - Marked `--duplicates-immich-native-algorithm` and `--duplicates-immich-native-deletion` as required selections for Immich `Remove Duplicates Assets` in Web, GUI, and TUI. Active native-detection commands now include both values explicitly; native deletion remains unavailable when detection is disabled.
...
#### 🐛 Bug fixes:
  ...
  - Fixed the Web Interface `Immich Remove Duplicates Assets` controls so `--duplicates-immich-native-algorithm` and `--duplicates-immich-native-deletion` are rendered as required selections, while an explicit `false` value is accepted and emitted instead of being mistaken for a missing argument.
  ...
  - Fixed Immich `Remove Duplicates Assets` argument consistency across the Web Interface, GUI, and TUI. Disabling `--duplicates-immich-native-algorithm` now explicitly forces `--duplicates-immich-native-deletion=false` in command previews, execution, and startup logging.
...
#### 📚 Documentation:
  ...
  - Added `Remove Duplicates Assets` and `--duplicates-asset-keeper {oldest, newest}` to the CLI syntax, full/short arguments references, and Automatic Migration guide, including its independent-module scope.
