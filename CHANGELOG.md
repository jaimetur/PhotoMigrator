...
#### 🐛 Bug fixes:
  ...
  - Fixed GUI/TUI interactive runtime regressions so `ExifTool` discovery now also falls back to executables available in the system `PATH`, the embedded `exif_tool` bundle auto-recovers missing Perl library files from `others.zip` before use, the CLI TUI `Execution Log` now uses an explicit drag-selection layer plus non-wrapped long lines for reliable selected-text copying and horizontal mouse-wheel scrolling, and the desktop GUI now clears persistent readonly `Combobox` text highlighting after a selection is made.
