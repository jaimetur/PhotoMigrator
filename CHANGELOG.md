...
#### 🚀 Enhancements:
  ...
  - Hardened Immich native duplicate resolution for partial group access. When Immich reports `Not found or no duplicate.delete access` for a batch, PhotoMigrator now retries only that batch's groups individually, continues resolving the accessible groups, and emits one final summary of inaccessible or stale group IDs instead of stopping the migration or flooding the log.
