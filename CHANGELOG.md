...
#### 🚀 Enhancements:
  ...
  - Optimized Google Takeout album-entry reconciliation. Step 5.5, now named `Reconcile Album Entries`, indexes an album once while processing all of its sidecars and updates that index in memory after creating entries, avoiding repeated recursive scans of the same album without weakening filename, date-folder, or byte-for-byte validation.
  - Clarified Google Takeout Step 5.5 reconciliation reporting. Successful physical-copy-to-shortcut replacements are now `INFO` rather than review warnings; repeated sidecars no longer repeat the same physical-entry warning; affected albums each emit one compact counter line; and the phase ends with a global, line-by-line summary separating evaluated sidecars, existing mappings, missing entries, recovered entries, repaired shortcuts, retained review items, unresolved entries, and affected albums.
  - Moved the visible Google Takeout output analysis after post-processing. Post-processing is now Phase 5, final disk analysis is Phase 6, and final cleanup remains Phase 7. The post-process date index is prepared before any moves and reused by Phase 6, which rescans the final filesystem for accurate physical-file, symlink, folder, and size totals without repeating the full metadata extraction. The help now notes that file explorers may omit symlinks from their file count.
  - Updated GPTH to v6.3.0 which includes new enhancements.
...
#### 🚨 Breaking Changes:
  - Separated cloud non-album folders from Takeout master libraries. `--foldername-no-albums` now defaults to `No_Albums` and is used only for assets without album association in cloud/local-library workflows. Google and iCloud Takeout now use the new `--foldername-all-photos` argument (default `ALL_PHOTOS`) for the master library containing every asset. Renamed `--google-no-albums-folders-structure` to `--google-all-photos-folders-structure` (`-gaps`) and `--icloud-no-albums-folders-structure` to `--icloud-all-photos-folders-structure` (`-iaps`); the removed flags are not accepted. Updated CLI, Web Interface, GUI, TUI, help, and Takeout processing to use the separate contract.
  - Automatic Migration now recognises configured `ALL_PHOTOS` roots, including `PARTNER_SHARED/ALL_PHOTOS`, as canonical Takeout source roots independently of `No_Albums`. Takeouts with no `Albums` directory therefore migrate their nested master-library assets correctly.
  ...
  - Renamed duplicate-asset arguments: `--immich-duplicates-algorithm` is now `--dup-immich-native-algorithm`, `--immich-duplicates-deletion` is now `--dup-immich-native-deletion`, and `--duplicate-asset-keeper` is now `--dup-asset-keeper`. The former long names are no longer accepted.
  - Renamed the managed local-library public contract: `--client=local-folder` is now `--client=local-photos-folder`, and `-localFolder, --local-folder` is now `-lPhotosFolder, --local-photos-folder`. The former client value and argument are no longer accepted; the guide is now `help/10-local-photos-folder.md`.

#### 🌟 New Features:
  - **Remove Duplicates Assets:** Added `--remove-duplicates-assets` to every cloud-service feature. Synology Photos, Immich Photos, and NextCloud Photos can remove duplicate physical assets using the required `--dup-asset-keeper` selector; Google Photos exposes the analysis and selection flow but clearly reports that its public Library API cannot delete media items. Immich additionally supports native visual duplicate detection and native metadata-preserving resolution, with PhotoMigrator's exact filename-and-size/manual-merge flow available as a fallback.
  - **Google Takeout People Extraction:** Added `--google-process-people=true|false` (enabled by default) to capture people labels from Google Takeout JSON sidecars during processing. It writes reusable `takeout_people_metadata.json` data keyed by filename and source dates, preserving independent same-name candidates for later date-based matching. The control is available in CLI, Web, GUI, and TUI.
  - **People Import for Supported Cloud Targets:** Added `--import-people` for Immich `Upload All`, `Upload Albums`, and Automatic Migration when the target is Immich. It consumes Google Takeout people metadata, matches assets by filename and nearest dates, and creates or reuses `people/<name>` tags without changing face assignments. Execution summaries report people assigned to at least one asset.
  - **Immich Burst Stack Control:** Added `--create-stacks=true|false` for Immich `Upload All`, `Upload Albums`, and Automatic Migration when the target is Immich. Stack creation and `--import-people` now default to enabled; Google Takeout keeps `--google-process-people` enabled by default. The controls are available in CLI, Web, GUI, and TUI.
  - **Cloud Album Consolidation:** Extended `--consolidate-albums-names` for Google Photos, Synology Photos, Immich Photos, and NextCloud Photos with compatible date-prefix detection and safe end-truncation detection. Date families retain the most precise compatible date as keeper; truncated names require a common dominant asset year and protect terminal Shared/Share/Public/Público variants from accidental merges with plain names. The detected keeper and merge candidates are previewed with `--preview-album-actions`, and `--request-user-confirmation` now confirms the displayed plan before consolidation.
  - **Local Photos Folder Management:** Added a Local Photos Folder tab after NextCloud Photos. It provides the common cloud-library modules against a managed on-disk library, with physical assets stored under `No_Albums` and album membership represented without duplicating media files.
  ...
  - Added Immich-native duplicate detection to `Remove Duplicates Assets` through `--dup-immich-native-algorithm=true|false` (default: enabled). The module uses Immich's visually similar duplicate groups by default; native deletion delegates the supported merge to Immich, while disabling it uses PhotoMigrator's guarded manual merge. `--dup-asset-keeper` adds the `better-quality` strategy, selected by default with native detection and based on Immich's suggested quality keeper; `oldest` and `newest` remain available.
  - Added the optional Immich-only `--dup-immich-native-deletion=true|false` flag to `Remove Duplicates Assets` across CLI, Web, TUI, and GUI. It defaults to enabled with `--dup-immich-native-algorithm=true`, and is disabled when native detection is off; when enabled, PhotoMigrator sends the selected keeper and redundant native duplicate assets to Immich's Alpha resolver, which merges its supported metadata and moves redundant assets to trash. The default without native detection keeps PhotoMigrator's guarded manual merge and permanent deletion path.
  ...
  - Added hover-revealed jump-to-first-line and jump-to-last-line controls to the Web Interface Output Log Panel. The top control switches to manual review, while the bottom control restores Auto Scroll and returns to the latest output.
  - Added people/tags-first duplicate keeper strategies for every cloud service: `more-people/tags-then-better-quality`, `more-people/tags-then-oldest`, and `more-people/tags-then-newest`. They retain the asset with the largest distinct people count, then the largest distinct tag count, before applying the named tie breaker; the quality variant is available only with Immich native duplicate detection. Immich defaults to `better-quality` with native detection and `more-people/tags-then-oldest` when native detection is disabled.
  - Refined Google Takeout people import during Immich Automatic Migration. Same-name map entries that cannot be distinguished because their dates are identical or unavailable now merge their labels instead of skipping import; asset logs show only non-empty found/assigned people context; `Album Pushed` summaries accumulate the labels found and confirmed assigned across all assets in that album, including duplicate assets resolved through the album-association queue; and the final summary reports assets with labels found, assets with labels confirmed assigned, and total unique people assigned.

#### 🚀 Enhancements:
  - Improved `Remove Duplicates Assets` inventory feedback across cloud backends. Immich uses `/api/search/statistics` for an exact user-visible total before its determinate 1,000-asset-page bar; Synology reads the native paginated result total for the same purpose; and NextCloud displays the real received-file count while traversing WebDAV without performing a wasteful second recursive scan, because WebDAV exposes no reliable recursive total. All scans now state clearly that large libraries can take several minutes; unsupported statistics endpoints fall back safely to indeterminate progress.
  ...
  - Optimized Immich `Remove Duplicates Assets` inventory retrieval by omitting person associations from the full paginated scan. Complete relationship metadata is now retrieved only for the small set of duplicate candidates before confirmation and deletion, retaining the face-safety check while avoiding an expensive people join and payload expansion for every library asset.
  ...
  - Restored Immich `Remove Duplicates Assets` inventory pages to 1,000 assets after compatibility testing showed that the target server rejects larger metadata pages.
  - Enhanced Immich `Remove Duplicates Assets` to preserve assigned people when deleting a duplicate group. The manual merge transfers faces only when every asset has the same Immich checksum and compatible normalized geometry, creating only faces missing from the keeper. Unsafe or unavailable face transfers are reported but no longer block deletion of the rest of the duplicate group.
  - Placed the `Immich Remove Duplicates Assets` account selector after its native-detection and keeper controls in the Web Interface, GUI, and TUI, and clarified that disabling native detection uses exact filename-and-size groups for cases such as repeated processed-Takeout uploads whose differing EXIF tag values prevented Immich duplicate rejection.
  - Renamed the Immich native duplicate-detection flag to `--dup-immich-native-algorithm`.
  - Expanded Immich `Remove Duplicates Assets` metadata preservation and its pre-deletion review. The preview now hydrates every candidate and reports visibility/archived state, capture date, coordinates, stack membership, and face references. Before deletion, the selected keeper preserves the most restrictive visibility (including archived assets), fills missing capture date and geolocation, and reconstructs affected stacks around the keeper; groups whose stack or metadata operations cannot be completed are skipped rather than deleted.
  - Linked the Immich duplicate-resolution controls across CLI, Web, TUI, and GUI: disabling `--dup-immich-native-algorithm` now disables `--dup-immich-native-deletion` and selects PhotoMigrator's manual flow; enabling native detection defaults the native-deletion control back to enabled.
  - Hardened the CLI dependency between the Immich duplicate flags: `--dup-immich-native-deletion` defaults to `true` with native detection, but an explicitly enabled native-deletion flag is rejected when `--dup-immich-native-algorithm=false`.
  - Prevented interactive Immich manual duplicate runs from emitting the disabled native-deletion option. With native detection off, Web, GUI, and TUI previews now omit `--dup-immich-native-deletion`; CLI validation also accepts an explicit `false` value while continuing to reject native deletion enabled without native detection.
  ...
  - Marked `--dup-immich-native-algorithm` and `--dup-immich-native-deletion` as required selections for Immich `Remove Duplicates Assets` in Web, GUI, and TUI. Active native-detection commands now include both values explicitly; native deletion remains unavailable when detection is disabled.
  ...
  - Added post-confirmation progress feedback to Immich `Remove Duplicates Assets`: manual cleanup reports group-by-group resolution, while native resolution reports duplicate-group preparation and real server-side resolution progress.
  - Added a blank log line before every `Remove Duplicates Assets` preview group header, improving separation between consecutive asset comparison tables.
  - Hardened Immich native duplicate resolution for partial group access. When Immich reports `Not found or no duplicate.delete access` for a batch, PhotoMigrator now subdivides only that batch until it isolates the inaccessible groups, keeps the accessible groups in server-side batches, and emits concise final summaries for inaccessible, stale (`not_found`), and other failed groups.
  - Optimized Immich manual `Remove Duplicates Assets` cleanup when `--dup-immich-native-algorithm=false`: metadata merges remain group-safe, but redundant asset IDs are now permanently deleted in 250-asset batches instead of one request per group. A rejected deletion batch is recursively isolated so valid IDs still delete and only the rejected IDs appear in one final warning with Immich's response. Per-group deletion messages are logged at `DEBUG`; normal output shows the progress bar, final totals, and separate metadata-merge versus deletion timings.
  ...
  - Refined Automatic Migration per-asset queue logging for dynamically populated staging folders. Push workers now atomically reserve the claimed asset and report the same physical staging state as `waiting`, `in flight`, and `completed/known` without changing transfer counters; album assets use `Album` / `Album Queue`, while non-album assets use their full input-relative `Folder` / `Folder Queue` scope. A duplicate response is recorded as completed for its source queue scope before any later album-association recovery, so moving it to `Album_Association_Queue` cannot make the displayed denominator decrease.
  ...
  - Added `Small Album Date Match` consolidation across Google Photos, Synology Photos, Immich Photos, NextCloud Photos, and Local Folder. An album with up to three assets can merge into a larger, similarly named album when its complete capture-date range fits inside the keeper range; the preview identifies this asset-date validation for each proposed merge.
  ...
  - Optimized Immich small-album date matching by reusing `AlbumResponseDto.assetCount`, `startDate`, and `endDate`. Full paginated asset reads are no longer performed for this rule; the album metadata directly determines whether the small range fits the keeper range.
  - Hardened Immich album-family consolidation metadata handling. A keeper retains its own non-empty description, otherwise receives the longest family description; direct sharing is restricted to users common to every album, with `viewer` selected whenever any source grants that more restrictive role. A public keeper link is removed whenever any source album has no public link. A metadata-reconciliation failure now leaves the family intact instead of removing a redundant album with unpreserved access or description data.
  - Made the small-album threshold configurable through `-smallAlbMax, --small-album-max-assets <COUNT>` (default `3`) for Consolidate Albums Names. The control appears directly below `--try-small-albums-grouping` in CLI help, Web, GUI, and TUI, is disabled and omitted from interactive commands when that detector is off, and applies consistently across Google Photos, Synology Photos, Immich Photos, NextCloud Photos, and Local Folder.
  - Expanded the Consolidate Albums Names final summary with a successful-family breakdown by matching rule, including equivalent names, date prefixes, truncated-name variants, and small-album date matches.
  - Improved Automatic Migration end-of-run diagnostics for retained work. Before the final summary, PhotoMigrator now lists every pull failure from `Pull_Failed/pull_failed_assets.csv` with its album and reason, plus every physical file left in `Push_Failed`, `Album_Association_Failed`, and pending push, delayed-push, or album-association queues. The final statistics now report the retained record/file count for each temporary subfolder.
  - Made Immich burst auto-stacking idempotent. Automatic Migration and Immich uploads now deduplicate repeated candidate groups discovered through multiple source albums and skip groups whose assets already belong to a remote stack, avoiding redundant `/api/stacks` requests and their misleading `400 Bad Request` warnings. Remaining stack-creation errors now include Immich's response body for direct diagnosis.
  - Added visible final-phase feedback for Immich burst auto-stacking. Automatic Migration and Immich uploads now announce the consecutive-capture stacking pass, show determinate progress by candidate group, and bound each stack-membership and stack-creation request to 30 seconds so a stalled Immich response cannot indefinitely delay the final summary.
  ...
  - Hardened Immich `Remove Duplicates Assets` for every keeper strategy except native `better-quality`. Native duplicate groups now use PhotoMigrator's guarded merge for `oldest`, `newest`, and every `more-people/tags-then-*` strategy: the selected keeper is re-read after metadata, album, and tag merging and redundant assets are sent to trash only when that keeper remains active and the requested merge is confirmed. Immich's Alpha deletion resolver is available only with native detection plus `better-quality`; when enabled, `better-quality` is the only allowed keeper. CLI, Web, GUI, and TUI disable the resolver for every other combination and restore the manual keeper choices when it is off. Native detection now defaults to `more-people/tags-then-better-quality`; disabling it continues to default to `more-people/tags-then-oldest`.
...
#### 🐛 Bug fixes:
  ...
  - Fixed Immich `Remove Duplicates Assets` detection to use the current `exifInfo.fileSizeInByte` response field, with fallback support for older size fields. Duplicate groups are no longer silently discarded when the legacy `fileSize` field is absent, and the analysis now reports assets that genuinely lack a usable size.
  ...
  - Fixed the Web Interface `Immich Remove Duplicates Assets` controls so `--dup-immich-native-algorithm` and `--dup-immich-native-deletion` are rendered as required selections, while an explicit `false` value is accepted and emitted instead of being mistaken for a missing argument.
  ...
  - Corrected Immich manual duplicate metadata merges to omit the invalid `rating=0` value, which Immich rejects with `400 Bad Request`. Any remaining metadata-merge failure now includes Immich's response body and the bounded asset-update payload in the warning for direct diagnosis.
  - Fixed Immich manual duplicate metadata merges to send lower-case API visibility values (`archive`, `timeline`, `hidden`, `locked`) instead of invalid upper-case values. Transient connection resets and timeouts during stack, asset, and tag merge requests now retry with exponential backoff up to five times; validation errors are not retried.
  - Fixed `Synology Photos` `Shared Space` assets without albums being omitted from `Download All` and `Automatic Migration` when asset filters were active (issue `#1173`). The global inventory now follows the browser-observed Shared Space timeline request: `POST /webapi/entry.cgi/SYNO.FotoTeam.Browse.Item`, `version=7`, `method=list`, with `start_time` / `end_time` filters. Its results are filtered using the configured criteria and merged by stable Synology asset ID with the personal-space inventory before assets already represented by albums are excluded. This retrieves the Shared Space `owner_user_id=0` assets that `SYNO.Foto.Browse.Item` does not expose.
  - Fixed the Web Interface omitting `--one-time-password` from Synology module commands. The OTP field is now included in the Synology schema as well as Automatic Migration, so an enabled 2FA control is rendered and passed to `Download All` and every other Synology module.
  - Fixed Synology Web Interface OTP prompts being hidden until a later line of process output arrived. Prompts without a trailing newline, including `Enter SYNOLOGY OTP Token:`, are now rendered immediately and mark the job input control as waiting. The web output now discards ANSI-coloured empty log prefixes and recognises ANSI-coloured `INFO` prefixes before replacing `tqdm` progress entries, preventing blank lines, duplicated `INFO` labels, and stale nested download progress bars. Synology now preserves `shared_with_me` scope, `normal_share_with_me` metadata, and passphrases when a shared album is also returned by the normal album list. True shared albums follow the browser-observed `POST SYNO.Foto.Browse.Item`, `version=7`, passphrase-only listing request (without `album_id`); Shared Space remains on its own Team endpoint and personal-space fallbacks remain available.
  - Fixed Synology Photos Shared Space non-album downloads for issue `#1173`. Assets enumerated through `SYNO.FotoTeam.Browse.Item` now retain their Team download provenance and use `SYNO.FotoTeam.Download`, with a controlled personal-API fallback for DSM variants that reject the Team endpoint. Successful HTTP responses containing JSON/HTML error bodies are rejected and removed instead of being saved as invalid media files.
  - Fixed Synology Photos `Download All` date filtering for issue `#1173`. Every personal and Shared Space inventory variant is now locally filtered before merging, preventing v7 fallback responses that ignore the legacy server-side time format from downloading personal assets outside the configured date range.
  - Fixed Immich `Remove Duplicates Assets` argument consistency across the Web Interface, GUI, and TUI. Disabling `--dup-immich-native-algorithm` now explicitly forces `--dup-immich-native-deletion=false` in command previews, execution, and startup logging.
  ...
  - Fixed Local Photos Folder Automatic Migration inventory aggregation to use the same album-selection rule as the puller and to preserve each album name when resolving its assets. Manifest-backed members, including Live Photo MOV companions in otherwise empty album folders, are now included during the initial analysis. Immich asset uploads now use bounded connection/read timeouts instead of allowing a stalled upload request to block migration workers indefinitely.
  ...
  - Hardened Immich stack operations in Automatic Migration and manual `Remove Duplicates Assets`. Burst groups now verify every destination asset before stack creation; a missing, stale, or non-writable candidate skips the complete group rather than issuing an invalid stack request. Manual duplicate stack preservation applies the same verification before recreating a stack and retains the duplicate group whenever verification fails. Warnings now identify affected filenames, include Immich's response once per group, and the final burst summary reports groups skipped because their destination assets could not be verified.
  - Fixed manual Immich duplicate stack preservation for redundant stack children. When duplicate metadata identifies another asset as the stack primary, PhotoMigrator now deletes only the redundant child and leaves that existing stack intact; it no longer performs an unnecessary `GET /stacks/{id}` that can fail on stale or inaccessible stack records.
  - Changed the default manual Immich duplicate keeper to `more-people/tags-then-oldest` when `--dup-immich-native-algorithm=false`. CLI, Web, TUI, GUI, runtime normalization, and help now use the same fallback; native detection continues to default to `better-quality`.
  - Fixed Windows Automatic Migration startup when a local source or processed Takeout folder is on a different drive from the bundled executable. Dashboard context formatting now falls back to the absolute path when Windows cannot compute a cross-drive relative path, without changing Docker's external-path restrictions. (Issue #1220)

#### 📚 Documentation:
  - Updated documentation with all changes.
  - Documented `Remove Duplicates Assets` in the cloud-feature matrix and all cloud-service guides, including keeper selection, confirmation, backend-specific metadata limitations, and deletion safeguards.
  ...
  - Added `Remove Duplicates Assets` and `--dup-asset-keeper {oldest, newest}` to the CLI syntax, full/short arguments references, and Automatic Migration guide, including its independent-module scope.
