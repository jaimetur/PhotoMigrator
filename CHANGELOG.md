...
#### 🐛 Bug fixes:
  ...
  - Fixed Web Interface command-preview state collection for checkbox flags so optional boolean flags such as `--google-keep-takeout-folder` now update the generated command reliably when toggled instead of potentially keeping a stale enabled value in the collected form payload. The shared/UI command builders were also covered with explicit tests for `Google Takeout` keep-takeout flag emission.
