...
#### 🐛 Bug fixes:
  ...
  - Fixed `Automatic Migration` Live Dashboard startup after its renderer extraction. Its estimated-time calculation, background progress parser, and visible-progress-row selection now reside in `LiveDashboard.py`, preventing `NameError` failures while rendering or consuming `tqdm` progress lines.
