...
- ### Main Changes:
  ...
  - #### 🐛 Bug fixes:
    ...
    - [x] Fixed a bug [#739](https://github.com/jaimetur/PhotoMigrator/issues/739) in function resolve_external_path() that after code refactoring on v3.4.0, the function was not resolving properly the paths when te tool were executed from compiled binary file.
...
- ### Main Changes:
  ...
  - #### 🐛 Bug fixes:
    - [x] Fixed a bug in function get_file_date() function affecting files with EXIF tags in different format (UTC naive and UTC aware). Now all EXIF date tags are converted to UTC aware before extracting the oldest date.
    - [x] Fixed a bug [#649](https://github.com/jaimetur/PhotoMigrator/issues/649) in function resolve_external_path() that after code refactoring on v3.4.0, the function was not resolving properly the paths when te tool were executed from compiled binary file.
