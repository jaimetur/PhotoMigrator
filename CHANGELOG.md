...
#### 🚀 Enhancements:
  ...
  - Added the optional Immich-only `--immich-duplicates-deletion=true|false` flag to `Remove Duplicate Assets` across CLI, Web, TUI, and GUI. It is disabled by default; when enabled, PhotoMigrator sends the selected keeper and redundant native duplicate assets to Immich's Alpha resolver, which merges its supported metadata and moves redundant assets to trash. The default keeps PhotoMigrator's guarded manual merge and permanent deletion path.
