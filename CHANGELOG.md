...
#### 🐛 Bug fixes:
  ...
  - Fixed `Google Takeout` `GPTH --fix` execution so PhotoMigrator now passes GPTH the direct parent folder of the album/asset subfolders instead of mixing `--fix` with `--input/--output`, relocates the generated `ALL_PHOTOS`, `Albums`, and `Special Folders` / `Special_Folders` trees into the configured PhotoMigrator output folder afterward, and when `--google-keep-takeout-folder` is active only clones a temporary working copy if the current `--fix` input still points to the original Takeout tree. If the source came from ZIP files and PhotoMigrator is already working on the intermediate unzipped staging folder, no extra clone is created because the original ZIP Takeout is already preserved. The automatic `album-only` detection flow still forces `--fix` when no year folders exist but localized `Google Photos` plus album JSON sidecars and `archive_browser.html` are present.
  - Fixed Web Interface command-preview state collection for checkbox flags so optional boolean flags such as `--google-keep-takeout-folder`, `--google-remove-duplicates-files`, `--google-rename-albums-folders`, `--google-skip-extras-files`, and the general `--no-log-file` flag now update the generated command reliably when toggled instead of potentially keeping a stale enabled value in the collected form payload. The preview payload builder now merges the latest in-memory UI state with the currently visible form values before calling `/api/preview`, and checkbox controls now also push their state directly into `stateValues` and trigger preview refreshes at the control level instead of relying only on the generic DOM binder. Extra deferred `click` synchronization was added as a browser-safety fallback for checkbox toggles activated through wrapping labels. The shared/UI command builders were also covered with explicit tests for these flag emissions.
