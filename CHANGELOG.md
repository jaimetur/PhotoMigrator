...
#### 🐛 Bug fixes:
  ...
  - Fixed the Rich `Automatic Migration` Live Dashboard finalization and layout: GUI-launched dashboards now report migration completion back to the GUI before silently waiting for a key, so final statistics remain visible while the GUI `Run` / `Stop` controls reset correctly. The Info, Pull, and Push panel dividers are now rendered as full-width Rich rules instead of split table-cell fragments.
