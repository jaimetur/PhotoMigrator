...
#### 🚀 Enhancements:
  ...
  - Optimized the Immich `Remove Duplicate Assets` inventory scan with explicit 1,000-asset metadata pages and a single indeterminate progress bar that reports received assets and rate without claiming the incorrect per-page total reported by some Immich versions. The initial scan now states clearly that a full-library analysis can take several minutes on large libraries instead of appearing stalled after authentication.
