...
#### 🚀 Enhancements:
  ...
  - Deferred `Automatic Migration` album association until the source album has finished pulling by default. Album-bound uploads are now accumulated during the hot pull/push phase and only flushed to the destination album once the puller marks that album as complete, still in bounded batches. This keeps the album-API work out of the per-asset hot path so `Pulled`/`Pushed` counters can advance faster inside large albums while preserving the final album membership checks and retries.
  ...
  - Reworked cloud `Automatic Migration` album handling across `Immich`, `Synology`, `Google Photos`, and the other cloud targets so the hot upload path now creates/reuses the exact working album first, associates assets immediately only when the target upload already returns a reusable destination asset id, and defers expensive duplicate-id resolution plus final canonical/similar-album consolidation to album-finalization time. Duplicate uploads rejected without a returned destination id are now kept in a per-album pending-resolution list and are resolved only once the source album finishes, which removes per-asset remote duplicate lookups from the main upload path while preserving correct album membership before the temporary album folder is cleaned up.
