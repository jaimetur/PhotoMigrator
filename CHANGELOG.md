...
#### 🚀 Enhancements:
  ...
  - Refined Automatic Migration per-asset `Album Queue` logging for dynamically populated staging folders. Push workers now atomically reserve the claimed asset and report the same physical staging state as `waiting`, `in flight`, and `completed/known` without changing transfer counters; completed is recorded only after a reusable destination asset ID exists, while retries and unresolved duplicates are released back to their pending flow.
