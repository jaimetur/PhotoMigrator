...
#### 🚀 Enhancements:
  ...
  - Moved `--prefer-canonical-album-names` and `--consolidate-similar-albums` out of the Web Interface `Configuration > General Arguments` panel into each applicable feature. Cloud modules and Automatic Migration keep their existing contextual controls, while Google Takeout and iCloud Takeout now expose both flags in their `Processing Flags` sections.
  - Applied the same contextual placement for `--prefer-canonical-album-names` and `--consolidate-similar-albums` in the TUI and desktop GUI: they no longer appear in General Arguments and are available in the Google Takeout and iCloud Takeout feature forms.
