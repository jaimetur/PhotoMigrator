...
#### 🐛 Bug fixes:
  ...
  - Fixed the pre-detection ZIP unpack step used by `Automatic Migration` local-folder sources so it now resolves the logger dynamically at runtime instead of crashing with `'NoneType' object has no attribute info/warning'` when `FileUtils` was imported before the global logger had been initialized.
