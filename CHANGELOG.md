...
#### 🐛 Bug fixes:
  ...
  - Fixed GUI/TUI interactive runtime regressions so `ExifTool` discovery now also falls back to executables available in the system `PATH`, the embedded `exif_tool` bundle auto-recovers missing Perl library files from `others.zip` before use, the CLI TUI `Execution Log` keeps long lines non-wrapped for real horizontal overflow/scroll inspection and better selected-text copying, and the desktop GUI now clears persistent readonly `Combobox` text highlighting after a selection is made.
