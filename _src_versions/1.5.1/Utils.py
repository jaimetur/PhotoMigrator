...
def find_duplicates(duplicates_action='list', find_duplicates_in_folders='./', deprioritize_folders_patterns=None, timestamp=None):
    ...
    if len(sizes_with_duplicates_dict)>0:
        ...
        if len(duplicates)>0:
            ...
            header = ['Num_Duplicates', 'Principal', 'Duplicate', 'Action', 'Reason for Principal']
            ...
            with open(duplicates_csv_path, 'w', encoding='utf-8-sig', newline='') as duplicates_file:
                ...
                for file_hash, duplicates_path_list in duplicates.items():
                    ...
                    for duplicated_path in sorted(duplicates_for_this_set):
                        row = [num_duplicates, principal, duplicated_path, display_action, reasson_for_principal]
                        if duplicates_action == 'move':
                            relative_path = None
                            source_root = None
                            for folder in input_folders_list:
                                if duplicated_path.startswith(folder):
                                    source_root = folder
                                    relative_path = os.path.relpath(duplicated_path, folder)
                                    break
                            if relative_path is None:
                                source_root = input_folders_list[0]
                                relative_path = os.path.relpath(duplicated_path, source_root)
                            ...
                            shutil.move(duplicated_path, destination_path)
                            row.append(destination_path)
                        elif duplicates_action == 'remove':
                            try:
                                os.remove(duplicated_path)
                            except OSError:
                                logger.warning(f"WARNING: Could not remove file {duplicated_path}")
