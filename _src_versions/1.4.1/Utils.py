...
def FindDuplicates(duplicates_action='List', find_duplicates_in_folders='./'):
    ...
    - A "Duplicates" directory and a timestamped subdirectory ("Duplicates_<timestamp>") are created,
      where a "Duplicates_<timestamp>.csv" file is written listing the duplicates.
      This CSV file will have the same timestamp as the directory.
    - The CSV file has:
      * First column: number of duplicate files (excluding the principal)
      * Second column: principal file
      * Third column: each duplicate file (one per line)
      * Fourth column: action ('keep' for list, 'move' for move, 'remove' for remove)
      * If action='move', a fifth column with the destination path for the moved duplicate
    - The function returns the number of duplicates found (total duplicated files excluding principals).
      This number matches the number of lines in the CSV (excluding the header).
    - If duplicates_action='move', duplicates except the principal are moved to the timestamped directory.
    - If duplicates_action='remove', duplicates except the principal are removed.
    - If duplicates_action='list', only listing is done.
    - If after moving or removing duplicates any empty directories remain in the original folders, they are removed.
    ...
    # CSV header
    # Col 1: Num_Duplicates (excluding principal)
    # Col 2: Principal
    # Col 3: Duplicate
    # Col 4: Action ('keep','move','remove')
    # Col 5: Destination (if action='move')
    header = ['Num_Duplicates', 'Principal', 'Duplicate', 'Action']
    if action == 'move':
        header.append('Destination')

    logger.info("INFO: Writing header to the duplicates CSV file.")
    with open(duplicates_csv_path, 'w', encoding='utf-8-sig', newline='') as duplicates_file:
        ...
        # duplicates_counter ahora representará el número total de duplicados (líneas en CSV sin contar encabezado)
        ...
        for file_hash, path_list in duplicates.items():
            ...
            logger.info(f"INFO: Principal file determined: {principal} with {num_duplicates} duplicates.")

            display_action = 'keep' if action == 'list' else action

            for duplicate_path in sorted(duplicates_for_this_set):
                row = [num_duplicates, principal, duplicate_path, display_action]
                if action == 'move':
                    relative_path = None
                    source_root = None
                    for folder in input_folders_list:
