...
def move_albums(input_folder, albums_subfolder="Albums", exclude_subfolder=None):
    ...
    for folder in os.listdir(input_folder):
        ...
        if (
            ...
            print(f"Moving to '{os.path.basename(albums_path)}' the folder: '{os.path.basename(folder_path)}'..")
...
def fix_metadata_with_gpth_tool(input_folder, output_folder, flatten_albums=True, flatten_no_albums=False, ignore_takeout_structure=False):
...
def copy_folder(src, dst, ignore_patterns=None):
    """
    Copies an entire folder, including subfolders and files, to another location,
...
if __name__ == "__main__":
    def parse_arguments():
        ...
        parser.add_argument("-it", "--ignore-takeout-structure", action="store_true", help="Ignore Google Takeout structure, and fix all files found on <TAKEOUT_FOLDER>.")
...
def main():
    ...
    if args.skip_gpth_tool or args.ignore_takeout_structure:
        ...
        step_start_time = datetime.now()
        copy_folder (args.takeout_folder, output_folder, ignore_patterns=['*.json', '*.j'])
        step_end_time = datetime.now()
        formatted_duration = str(timedelta(seconds=(step_end_time-step_start_time).seconds))
        logger.info(f"INFO: Step 2 completed in {formatted_duration}.")
    ...
    if not args.skip_move_albums:
        step_start_time = datetime.now()
        move_albums(output_folder, exclude_subfolder=["ALL_PHOTOS", "@eaDir"])
        step_end_time = datetime.now()
        formatted_duration = str(timedelta(seconds=(step_end_time-step_start_time).seconds))
